...
final class ShareViewController: UIViewController {
  ...
  private func setupUI() {
    ...
    NSLayoutConstraint.activate([
      self.draftTextView.topAnchor.constraint(
        equalTo: self.view.safeAreaLayoutGuide.topAnchor, constant: 14),
      self.draftTextView.leadingAnchor.constraint(equalTo: self.view.leadingAnchor, constant: 14),
      self.draftTextView.trailingAnchor.constraint(
        equalTo: self.view.trailingAnchor, constant: -14),
      ...
      buttons.bottomAnchor.constraint(
        equalTo: self.view.keyboardLayoutGuide.topAnchor, constant: -8),
  ...
  private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws
  {
    ...
    let params = AgentRequestPayload(
      ...
      receiptText: canDeliverToRoute
        ? "Just received your iOS share + request, working on it." : nil,
    ...
    let eventData = try JSONEncoder().encode(
      NodeEventParams(event: "agent.request", payloadJSON: json))
    ...
    _ = try await gateway.request(
      method: "node.event", paramsJSON: nodeEventParams, timeoutSeconds: 25)
  }
      ...
      for provider in item.attachments ?? [] {
        ...
        if provider.hasItemConformingToTypeIdentifier(UTType.image.identifier) {
          ...
          if attachments.count < maxImageAttachments,
            let attachment = await self.loadImageAttachment(
              from: provider, index: attachments.count)
          {
  ...
  private func loadImageAttachment(from provider: NSItemProvider, index: Int) async
    -> ShareAttachment?
  {
  ...
  private func loadURL(from provider: NSItemProvider) async -> URL? {
    ...
    if provider.hasItemConformingToTypeIdentifier(UTType.text.identifier) {
      if let text = await self.loadTextValue(
        from: provider, typeIdentifier: UTType.text.identifier),
