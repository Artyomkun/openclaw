...
struct AppleReviewDemoChatTransport: OpenClawChatTransport {
  ...
  func createSession(
    ...
    try await self.transport.createSession(
      key: key, label: label, parentSessionKey: parentSessionKey)
  }
  ...
  func setSessionThinking(sessionKey: String, thinkingLevel: String) async throws {
    try await self.transport.setSessionThinking(
      sessionKey: sessionKey, thinkingLevel: thinkingLevel)
  }
...
private actor LocalFixtureChatStore {
  ...
  func history(sessionKey: String) throws -> OpenClawChatHistoryPayload {
    let normalizedSessionKey = Self.normalizedSessionKey(
      sessionKey, fallback: self.fixture.sessionKey)
  ...
  func sendMessage(sessionKey _: String, message: String, runId: String) throws
