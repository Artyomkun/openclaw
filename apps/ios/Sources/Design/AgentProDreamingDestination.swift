...
struct AgentProDreamingDestination: View {
        ...
        HStack {
          VStack(alignment: .leading, spacing: 3) {
            Text("Maintenance")
              .font(.headline)
            Text(
              "Refresh reads live state. Maintenance actions update the gateway diary/artifacts."
            )
            .font(.caption)
            .foregroundStyle(.secondary)
            .lineLimit(2)
        ...
        HStack(spacing: 8) {
          ForEach(DreamAction.allCases) { action in
            Button {
              ...
              Label(
                action.title,
                systemImage: self.dreamActionBusy == action ? "hourglass" : action.icon)
            }
            .buttonStyle(.bordered)
            .controlSize(.small)
      ...
      ProCard(padding: 0) {
        if entries.isEmpty {
          self.emptyDetailRow(
            ...
            title: emptyTitle,
            detail: self.gatewayConnected
              ? emptyDetail : "Connect a gateway to load dreaming entries."
          )
          .padding(14)
        } else {
          VStack(spacing: 0) {
  ...
  private func dreamingPhaseRow(_ phase: DreamingPhaseRow) -> some View {
    HStack(alignment: .top, spacing: 12) {
      ...
      Text(self.dreamingPhaseState(phase.status))
        .font(.caption2.weight(.semibold))
        .foregroundStyle(
          phase.status.managedCronPresent == true ? OpenClawBrand.accent : .secondary
        )
        .lineLimit(1)
    }
    .padding(.vertical, 10)
  ...
  private static func dreamDiaryDays(from content: String) -> [DreamDiaryDay] {
    ...
    let separatorBlocks =
      ...
      .filter { !$0.isEmpty }
    let blocks =
      separatorBlocks.count > 1 ? separatorBlocks : Self.splitDiaryBlocksByDateLine(inner)
