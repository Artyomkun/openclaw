...
extension AgentProTab {
  ...
  func cronJobState(_ job: CronJob) -> String {
    ...
    if let status = Self.stringValue(job.state["lastStatus"])
      ?? Self.stringValue(job.state["lastRunStatus"])
    {
      return status
    }
    return "enabled"
  ...
  func refreshOverview(force: Bool) async {
    ...
    async let cronJobs = self.requestOptional(
      ...
      method: "cron.list",
      paramsJSON:
        "{\"includeDisabled\":true,\"limit\":8,\"sortBy\":\"nextRunAtMs\",\"sortDir\":\"asc\"}",
      timeoutSeconds: 12)
    async let dreaming = self.requestOptional(
      DreamingStatusEnvelope.self, method: "doctor.memory.status")
    async let dreamDiary = self.requestOptional(
      DreamDiaryLite.self, method: "doctor.memory.dreamDiary")
