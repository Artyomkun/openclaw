...
extension AgentProTab {
          ...
          Button {
            ...
            Text(filter.title)
              ...
              .overlay {
                Capsule()
                  .strokeBorder(
                    Color.primary.opacity(self.agentRosterFilter == filter ? 0.22 : 0.06))
              }
          }
          .buttonStyle(.plain)
        }
  ...
  func agentAvatar(_ agent: AgentSummary, state: AgentRosterState) -> some View {
    ZStack(alignment: .bottomTrailing) {
      Text(self.agentBadge(for: agent))
        .font(
          .system(
            size: self.agentBadge(for: agent).count > 2 ? 14 : 18, weight: .bold, design: .rounded)
        )
  ...
  var liveGatewayConnected: Bool {
    !self.appModel.isLocalGatewayFixtureEnabled && self.gatewayConnected
  ...
  var emptyAgentsTitle: String {
    ...
    if self.agentRosterFilter != .all {
      return "No \(self.agentRosterFilter.title.lowercased()) agents"
    }
  ...
  var skillsDetail: String {
    ...
    guard let skills = self.overview?.skills else {
      return self.overviewLoading
        ? "Loading skill status." : "Skill status is available from the gateway."
    }
  ...
  var dreamingDetail: String {
    ...
    guard let dreaming = self.overview?.dreaming else {
      return self.overviewLoading
        ? "Loading dreaming status." : "Background memory status is available."
    }
    if let nextRunAtMs = dreaming.nextRunAtMs {
      return "Next cycle \(Self.relativeTime(fromMilliseconds: nextRunAtMs))"
    }
    return
      "\(dreaming.totalSignalCount ?? 0) signals, \(dreaming.promotedToday ?? 0) promoted today"
  }
  ...
  var recentCronJobs: [CronJob] {
    (self.overview?.cronJobs ?? [])
      .sorted { lhs, rhs in
        ...
        case (let lhsNext?, let rhsNext?): return lhsNext < rhsNext
