...
extension AgentProTab {
  ...
  func skillRow(_ skill: SkillStatusEntryLite) -> some View {
    ...
    return HStack(alignment: .top, spacing: 12) {
      ProIconBadge(
        systemName: self.isSkillAllowed(skill) ? "checkmark.circle" : "nosign", color: status.color)
      VStack(alignment: .leading, spacing: 4) {
        Text(skill.displayName)
          .font(.subheadline.weight(.semibold))
          .lineLimit(1)
        Text(
          self.normalized(skill.description) ?? self.normalized(skill.source) ?? "Workspace skill"
        )
        .font(.caption)
        .foregroundStyle(.secondary)
        .lineLimit(2)
  ...
  func installClawHubSkill(_ result: ClawHubSearchResultLite) async {
    ...
    do {
      ...
      _ = try await self.requestGateway(
        method: "skills.install", params: params, timeoutSeconds: 125)
      await self.appModel.refreshGatewayOverviewIfConnected()
      await self.refreshOverview(force: true)
    } catch {
  ...
  func searchClawHubSkills() async {
    ...
    do {
      ...
      let data = try await self.requestGateway(
        method: "skills.search", params: params, timeoutSeconds: 20)
      self.clawHubResults = try JSONDecoder().decode(ClawHubSearchResponseLite.self, from: data)
        .results
    } catch {
      self.clawHubErrorText = Self.skillMutationMessage(error)
    }
