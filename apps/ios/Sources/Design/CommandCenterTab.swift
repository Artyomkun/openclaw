...
struct CommandCenterTab: View {
      ...
      VStack(alignment: .leading, spacing: 10) {
        ...
        .background {
          RoundedRectangle(cornerRadius: 10, style: .continuous)
            .fill(
              self.colorScheme == .dark ? Color.black.opacity(0.16) : Color.black.opacity(0.026)
            )
            .overlay {
              RoundedRectangle(cornerRadius: 10, style: .continuous)
                .strokeBorder(
      ...
      VStack(spacing: 10) {
        ...
        if self.recentSessionPreviewRows.isEmpty {
          CommandEmptyStateRow(
            ...
            detail: self
              .gatewayConnected
              ? "Start a chat and it will appear here." : "Connect to the gateway.")
        } else {
          VStack(spacing: 8) {
  ...
  private var gatewayStateText: String {
    ...
    let status = self.appModel.gatewayDisplayStatusText.trimmingCharacters(
  ...
  private static func sessionChoices(
    ...
    for session in sorted {
      ...
      guard Self.isRecentChatSession(session.key, defaultSessionKey: defaultSessionKey) else {
        continue
      }
  ...
  private nonisolated static func isAgentDeviceSession(_ key: String, defaultSessionKey: String)
    -> Bool
  {
    let parts =
      key
...
struct CommandSessionsScreen: View {
        ...
        if let loadErrorText {
          ...
          CommandEmptyStateRow(
            icon: self.appModel
              ...
              ? "bubble.left.and.text.bubble.right.fill" : "wifi.slash",
            title: self.appModel.isCommandSessionListAvailable
              ? "No recent sessions" : "Gateway offline",
  ...
  private var sessionRows: [CommandCenterTab.WorkItem] {
    self.sessions
      .filter {
        CommandCenterTab.isRecentChatSession(
          $0.key,
          defaultSessionKey: self.appModel.defaultChatSessionKey)
      }
