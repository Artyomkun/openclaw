...
struct IPadSkillWorkshopScreen: View {
  ...
  init(
    headerLeadingAction: OpenClawSidebarHeaderAction? = nil, openSettings: @escaping () -> Void = {}
  ) {
  ...
  private var defaultAgentScopeLabel: String {
    let defaultID = Self.normalizedScopeID(self.appModel.gatewayDefaultAgentId)
    if let match = appModel.gatewayAgents.first(where: {
      Self.normalizedScopeID($0.id) == defaultID
    }) {
  ...
  static let defaultProposalStatusBoardLanes = [
    "pending", "quarantined", "stale", "applied", "rejected",
  ]
  ...
  static func nextSelectedProposalID(
    ...
    let filtered = proposals.filter {
      Self.proposalStatusMatchesFilter(status: $0.status, filter: filter)
    }
  ...
  private func inspect(proposalID: String, force: Bool) async {
    ...
    guard force || self.proposals.first(where: { $0.id == proposalID })?.content == nil else {
      return
    }
    ...
    do {
      ...
      self.merge(
        IPadSkillProposal(inspect: response, previous: self.proposals.first { $0.id == proposalID })
      )
    } catch {
      self.errorText = Self.message(for: error)
    }
...
struct IPadSkillProposalRow: View {
  ...
  var body: some View {
    HStack(alignment: .top, spacing: 12) {
      ProIconBadge(
        systemName: self.isBusy ? "hourglass" : "hammer", color: self.proposal.statusColor)
      VStack(alignment: .leading, spacing: 4) {
        Text(self.proposal.title)
          .font(.subheadline.weight(.semibold))
          .foregroundStyle(self.isSelected ? OpenClawBrand.accent : .primary)
...
struct IPadSkillProposal: Identifiable {
  ...
  init(entry: IPadSkillProposalManifestEntry, previous: IPadSkillProposal?) {
    ...
    self.supportFiles =
      previous?.updatedAtMs == self.updatedAtMs ? previous?.supportFiles ?? [] : []
  }
  ...
  private static func parseDate(_ value: String) -> Double {
    (ISO8601DateFormatter().date(from: value)?.timeIntervalSince1970 ?? Date().timeIntervalSince1970)
      * 1000
  }
