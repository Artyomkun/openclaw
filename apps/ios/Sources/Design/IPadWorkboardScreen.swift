...
struct IPadWorkboardScreen: View {
  ...
  private var selectedBoardParam: String? {
    Self.normalizedScopeID(self.selectedBoardID).isEmpty
      ? nil : Self.normalizedScopeID(self.selectedBoardID)
  }
  ...
  static func boardScopeOptions(knownBoardIDs: [String], cardBoardIDs: [String]) -> [String] {
    Array(
      Set((knownBoardIDs + cardBoardIDs).map { self.normalizedScopeID($0) }.filter { !$0.isEmpty })
    )
    .sorted()
  }
  ...
  private func cards(forKanbanStatus status: String) -> [IPadWorkboardCard] {
    self.cards
      .filter { card in
        card.status == status
          && (self.selectedStatus != "active" || card.metadata?.archivedAt == nil)
      }
...
private struct IPadWorkboardKanbanCard: View {
    ...
    VStack(alignment: .leading, spacing: 10) {
      ...
      HStack(spacing: 8) {
        ...
        Menu {
          ...
          Button(
            self.card.metadata?.archivedAt == nil ? "Archive" : "Unarchive", action: self.archive)
        } label: {
  ...
  private var detail: String {
    ...
    if let sessionKey = card.sessionKey?.trimmingCharacters(in: .whitespacesAndNewlines),
      !sessionKey.isEmpty
    {
      return sessionKey
    }
    ...
    NavigationStack {
      Form {
        Section("Card") {
          ...
          if let notes = self.card.notes?.trimmingCharacters(in: .whitespacesAndNewlines),
            !notes.isEmpty
          {
            Text(notes)
          }
