...
struct SettingsChannelsDestination: View {
      ...
      VStack(spacing: 0) {
        ...
        if let errorText {
          ...
          ProStatusRow(
            ...
            detail:
              "Connect to the gateway to load installed channels, accounts, and routing status.",
            value: "offline",
            color: .secondary)
  ...
  private var summaryColor: Color {
    ...
    return self.channelEntries.contains(where: { $0.running || $0.connected })
      ? OpenClawBrand.ok
      : OpenClawBrand
        .accent
  }
  ...
  private var channelEntries: [SettingsChannelEntry] {
    ...
    let ids =
      snapshot.channelorder.isEmpty ? Array(snapshot.channels.keys).sorted() : snapshot.channelorder
  ...
  private func entry(channelID: String, snapshot: ChannelsStatusResult) -> SettingsChannelEntry {
    ...
    let configured =
      accounts.contains(where: \.configured) || summary["configured"]?.boolValue == true
    ...
    let detail =
      snapshot.channeldetaillabels?[channelID]?.stringValue ?? Self.fallbackDetail(channelID)
    let systemImage =
      snapshot.channelsystemimages?[channelID]?.stringValue ?? Self.fallbackSystemImage(channelID)
  ...
  private func accounts(channelID: String, snapshot: ChannelsStatusResult)
  ...
  private func loadChannels(force: Bool) async {
    ...
    do {
      ...
      let data = try await self.request(
        method: "channels.status", params: params, timeoutSeconds: 12)
  ...
  private func run(_ kind: SettingsChannelOperation.Kind, channelID: String, accountID: String?)
    async
  {
    guard
      Self.shouldEnableChannelOperation(
        canRead: self.canRead, hasOperatorAdminScope: self.canAdmin),
      self.busyOperation == nil
    else {
      return
    }
    self.busyOperation = SettingsChannelOperation(
  ...
  private func request(method: String, params: some Encodable, timeoutSeconds: Int) async throws
    -> Data
  {
