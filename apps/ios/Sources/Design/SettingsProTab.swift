...
struct SettingsProTab: View {
  ...
  @AppStorage("location.enabledMode") var locationModeRaw: String = OpenClawLocationMode.off
    .rawValue
  ...
  @AppStorage(TalkModeRealtimeVoiceSelection.storageKey) var talkRealtimeVoiceSelectionRaw: String =
    ""
  @AppStorage(TalkSpeechLocale.storageKey) var talkSpeechLocale: String = TalkSpeechLocale
    .automaticID
  ...
  private func settingsModalPresentation(_ content: some View) -> some View {
    content
      ...
      .alert(
        "Enable OpenClaw Hosted Push Relay?", isPresented: self.$showNotificationRelayDisclosure
      ) {
        Button("Continue") {
