...
extension SettingsProTab {
  ...
  func syncSettingsState() {
    ...
    self.gatewayPassword =
      GatewaySettingsStore.loadGatewayPassword(instanceId: trimmedInstanceId) ?? ""
  }
  ...
  func applyPendingGatewaySetupLinkIfNeeded() {
    ...
    self.setupStatusText =
      "Setup link loaded for \(link.host):\(link.port) (\(security)). Tap Connect to apply."
  }
  ...
  func applyGatewayLink(_ link: GatewayConnectDeepLink) {
    ...
    if setupAuth.hasBootstrapToken {
      GatewayOnboardingReset.prepareForBootstrapPairing(
        appModel: self.appModel, instanceId: instanceId)
    }
    if !instanceId.isEmpty {
      GatewaySettingsStore.saveGatewayBootstrapToken(
        setupAuth.bootstrapToken, instanceId: instanceId)
    }
  ...
  func preflightGateway(host: String, port: Int) async -> Bool {
    ...
    let ok = await TCPProbe.probe(
      host: trimmed, port: port, timeoutSeconds: 3, queueLabel: "gateway.preflight")
    if !ok {
  ...
  func requestNotificationAuthorizationFromSettings() {
    ...
    Task {
      let granted =
        await
        (try? UNUserNotificationCenter.current().requestAuthorization(options: [
          .alert,
          .badge,
          .sound,
        ])) ?? false
  ...
  var setupStatusLine: String? {
    ...
    let gatewayStatus = self.appModel.gatewayStatusText.trimmingCharacters(
  ...
  var notificationRelayDetail: String {
    if PushBuildConfig.current.usesOpenClawHostedRelay {
      let host =
        PushBuildConfig.current.relayBaseURL.flatMap {
