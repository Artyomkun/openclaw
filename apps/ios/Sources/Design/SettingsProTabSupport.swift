  ...
  private struct SettingsGatewayStatesPreview: View {
    ...
    private func gatewayActionsCard(isBusy: Bool) -> some View {
      ProCard(radius: SettingsLayout.cardRadius) {
        HStack(spacing: 10) {
          self.previewButton(
            "Reconnect", systemImage: "arrow.triangle.2.circlepath", isBusy: isBusy)
    ...
    private var setupActionsCard: some View {
      ProCard(radius: SettingsLayout.cardRadius) {
        VStack(alignment: .leading, spacing: 10) {
          ...
          Text(
            "Discovered gateways and manual setup live here when the gateway has not connected yet."
          )
          .font(.caption)
          .foregroundStyle(.secondary)
        }
