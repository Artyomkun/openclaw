...
struct TalkProTab: View {
  ...
  @AppStorage(TalkSpeechLocale.storageKey) private var talkSpeechLocale: String = TalkSpeechLocale
    .automaticID
    ...
    CommandPanel(padding: 0) {
      VStack(spacing: 0) {
        self.cardHeader(title: "Conversation", value: self.state.chipText, color: self.state.color)
          ...
          .padding(.bottom, 3)
        self.infoRow(
          icon: "person.crop.circle.fill", title: "Agent", value: self.appModel.chatAgentName)
        ...
        Divider().padding(.leading, 54)
        self.infoRow(
          icon: self.state.icon, title: "Runtime", value: self.appModel.talkMode.statusText)
      }
    }
    .padding(.horizontal, OpenClawProMetric.pagePadding)
  }
  ...
  private var voiceModeCard: some View {
    CommandPanel(padding: 0) {
      VStack(spacing: 0) {
        ...
        Divider().padding(.leading, 54)
        self.infoRow(
          icon: "antenna.radiowaves.left.and.right", title: "Transport", value: self.transportText)
  ...
  private var headerSubtitle: String {
    let mode = self.appModel.talkMode.gatewayTalkVoiceModeTitle.trimmingCharacters(
  ...
  private var transportText: String {
    let provider = self.appModel.talkMode.gatewayTalkProviderLabel.trimmingCharacters(
      in: .whitespacesAndNewlines)
    let transport = self.appModel.talkMode.gatewayTalkTransportLabel.trimmingCharacters(
      in: .whitespacesAndNewlines)
  ...
  private var activeModeText: String {
    let title = self.appModel.talkMode.gatewayTalkActiveModeTitle.trimmingCharacters(
      in: .whitespacesAndNewlines)
...
private struct TalkProWaveform: View {
  ...
  private func amplitude(for index: Int, date: Date) -> Double {
    if self.reduceMotion {
      switch self.mode {
      case .level(let level): return min(max(level, 0.10), 1.0)
