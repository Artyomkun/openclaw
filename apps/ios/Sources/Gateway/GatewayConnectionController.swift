...
final class GatewayConnectionController {
  ...
  private func connectDiscoveredGateway(
    ...
    let tlsParams = stored.map { fp in
      GatewayTLSParams(
        required: true, expectedFingerprint: fp, allowTOFU: false, storeKey: stableID)
    }
  ...
  func acceptPendingTrustPrompt() async {
    ...
    if pending.isManual {
      ...
      GatewaySettingsStore.saveLastGatewayConnectionDiscovered(
        stableID: pending.stableID, useTLS: true)
    }

    let instanceId = GatewaySettingsStore.currentInstanceID()
    let token =
      pending.authOverride.map(\.token)
      ?? GatewaySettingsStore.loadGatewayToken(instanceId: instanceId)
    let bootstrapToken =
      pending.authOverride.map(\.bootstrapToken)
      ?? GatewaySettingsStore.loadGatewayBootstrapToken(
        instanceId: instanceId)
    let password =
      pending.authOverride.map(\.password)
      ?? GatewaySettingsStore.loadGatewayPassword(instanceId: instanceId)
  ...
  private func maybeAutoConnect() {
    ...
    let instanceId =
      defaults.string(forKey: "node.instanceId")?
      .trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
    ...
    if manualEnabled {
      let manualHost =
        defaults.string(forKey: "gateway.manual.host")?
        .trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
      ...
      guard
        let resolvedPort = self.resolveManualPort(
          host: manualHost,
          port: manualPort,
  ...
  private func startLastKnownAutoConnect(
    ...
    switch lastKnown {
    case .manual(let host, let port, let useTLS, let stableID):
      let resolvedUseTLS = self.resolveManualUseTLS(host: host, useTLS: useTLS)
      let stored = GatewayTLSStore.loadFingerprint(stableID: stableID)
      let tlsParams = stored.map { fp in
        GatewayTLSParams(
  ...
  private func savedManualEndpointFallback(defaults: UserDefaults = .standard)
    -> SavedManualEndpoint?
  {
  ...
  private func resolveServiceEndpoint(_ endpoint: NWEndpoint) async -> (host: String, port: Int)? {
    ...
    return await withCheckedContinuation { continuation in
      let resolver = GatewayServiceResolver(name: name, type: type, domain: domain) {
        [weak self] result in
        Task { @MainActor in
...
extension GatewayConnectionController {
  ...
  private static func isLocationAvailable(servicesEnabled: Bool, status: CLAuthorizationStatus)
    -> Bool
  {
  ...
  extension GatewayConnectionController {
    ...
    static func _test_isLocationAvailable(servicesEnabled: Bool, status: CLAuthorizationStatus)
      -> Bool
    {
...
private final class GatewayTLSFingerprintProbe: NSObject, URLSessionDelegate, @unchecked Sendable {
  ...
  func start() {
    ...
    DispatchQueue.global(qos: .utility).asyncAfter(deadline: .now() + self.timeoutSeconds) {
      [weak self] in
      self?.finish(nil)
    }
  }
  ...
  private func finish(_ fingerprint: String?) {
    ...
    let (shouldComplete, taskToCancel, sessionToInvalidate) = self.state.withLock {
      s -> FinishState in
