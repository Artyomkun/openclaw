...
enum GatewayConnectionIssue: Equatable {
  ...
  var requestId: String? {
    if case .pairingRequired(let requestId) = self {
      return requestId
    }
    return nil
  }
  ...
  static func detect(from statusText: String) -> Self {
    ...
    if lower.contains("pairing required") || lower.contains("not_paired")
    ...
    if lower.contains("connection refused") || lower.contains("timed out")
      || lower.contains("network is unreachable") || lower.contains("cannot find host")
  ...
  private static func extractRequestId(from statusText: String) -> String? {
    ...
    let end =
      trimmed.firstIndex(where: { ch in
