...
enum GatewaySettingsStore {
  ...
  static func loadStableInstanceID() -> String? {
    if let value = KeychainStore.loadString(
      service: self.nodeService, account: self.instanceIdAccount)?
      .trimmingCharacters(in: .whitespacesAndNewlines),
      !value.isEmpty
    {
      return value
  ...
  static func saveStableInstanceID(_ instanceId: String) {
    _ = KeychainStore.saveString(
      instanceId, service: self.nodeService, account: self.instanceIdAccount)
  }

  static func loadPreferredGatewayStableID() -> String? {
    if let value = KeychainStore.loadString(
      service: self.gatewayService,
      account: self.preferredGatewayStableIDAccount)?.trimmingCharacters(
  ...
  static func loadLastDiscoveredGatewayStableID() -> String? {
    if let value = KeychainStore.loadString(
      service: self.gatewayService,
      account: self.lastDiscoveredGatewayStableIDAccount)?.trimmingCharacters(
        in: .whitespacesAndNewlines),
      !value.isEmpty
    {
      return value
  ...
  static func saveLastGatewayConnectionManual(
    host: String, port: Int, useTLS: Bool, stableID: String
  ) {
  ...
  static func loadLastGatewayConnection() -> LastGatewayConnection? {
    // Migrate old UserDefaults entries on first access.
  ...
  static func clearLastGatewayConnection(defaults: UserDefaults = .standard) {
    _ = KeychainStore.delete(
      service: self.gatewayService, account: self.lastGatewayConnectionAccount)
    // Clean up any old UserDefaults entries.
  ...
  /// Migrate old UserDefaults gateway.last.* keys into a single Keychain entry.
  private static func migrateLastGatewayFromUserDefaultsIfNeeded() {
    ...
    let kindRaw =
      defaults.string(forKey: self.lastGatewayKindDefaultsKey)?
      .trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
...
enum GatewayDiagnostics {
  ...
  private static let logger = Logger(
    subsystem: "ai.openclawfoundation.app", category: "GatewayDiag")
