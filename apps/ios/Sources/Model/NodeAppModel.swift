...
final class NodeAppModel {
  ...
  private let pendingActionLogger = Logger(
    subsystem: "ai.openclawfoundation.app", category: "PendingAction")
  private let locationWakeLogger = Logger(
    subsystem: "ai.openclawfoundation.app", category: "LocationWake")
  private let watchReplyLogger = Logger(
    subsystem: "ai.openclawfoundation.app", category: "WatchReply")
  private let watchExecApprovalLogger = Logger(
    subsystem: "ai.openclawfoundation.app", category: "WatchExecApproval")
  ...
  private(set) var pendingNotificationPermissionGuidancePrompt:
    NotificationPermissionGuidancePrompt?
  ...
  @ObservationIgnored private lazy var capabilityRouter: NodeCapabilityRouter =
    self.buildCapabilityRouter()
  ...
  init(
    ...
    self.apnsDeviceTokenHex = UserDefaults.standard.string(
      forKey: Self.apnsDeviceTokenUserDefaultsKey)
  ...
  private func handleCanvasA2UIAction(body: [String: Any]) async {
    ...
    let surfaceId: String = {
      let raw =
        (userAction["surfaceId"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
    ...
    let instanceId = (UserDefaults.standard.string(forKey: "node.instanceId") ?? "ios-node")
      .lowercased()
    ...
    if await !self.isGatewayConnected() {
      ...
      do {
        try await self.sendAgentRequest(
          link: AgentDeepLink(
            message: message,
    ...
    let js = OpenClawCanvasA2UIAction.jsDispatchA2UIActionStatus(
      actionId: actionId, ok: ok, error: errorText)
    do {
  ...
  func setScenePhase(_ phase: ScenePhase) {
    ...
    case .background:
      ...
      self.backgroundTalkSuspended = self.talkMode.suspendForBackground(
        keepActive: shouldKeepTalkActive)
    case .active, .inactive:
      ...
      if phase == .active {
        self.voiceWake.resumeAfterExternalAudioCapture(
          wasSuspended: self.backgroundVoiceWakeSuspended)
        ...
        Task { [weak self] in
          ...
          await self.talkMode.resumeAfterBackground(
            wasSuspended: suspended, wasKeptActive: keptActive)
        }
        Task { [weak self] in
        ...
        if backgroundedFor >= 3.0 {
          ...
          Task { [weak self] in
            ...
            if operatorWasConnected {
              ...
              let healthy =
                await
                (try? self.operatorGateway.request(
                  method: "health",
                  paramsJSON: nil,
  ...
  private func beginBackgroundConnectionGracePeriod(seconds: TimeInterval = 25) {
    ...
    let taskID = UIApplication.shared.beginBackgroundTask(withName: "gateway-background-grace") {
      [weak self] in
      Task { @MainActor in
  ...
  private func refreshBrandingFromGateway() async {
    do {
      let res = try await self.operatorGateway.request(
        method: "config.get", paramsJSON: "{}", timeoutSeconds: 8)
  ...
  private func refreshAgentsFromGateway() async {
    do {
      let res = try await self.operatorGateway.request(
        method: "agents.list", paramsJSON: "{}", timeoutSeconds: 8)
  ...
  func refreshGatewayOverviewIfConnected() async {
    ...
    if self.foregroundGatewayResumeCheckInFlight {
      ...
      guard await self.isOperatorConnected(), !self.foregroundGatewayResumeCheckInFlight else {
        return
      }
    }
  ...
  func setSelectedAgentId(_ agentId: String?) {
    ...
    let currentSelectedAgentId = self.selectedAgentId?.trimmingCharacters(
    ...
    if stableID.isEmpty {
      ...
      GatewaySettingsStore.saveGatewaySelectedAgentId(
        stableID: stableID, agentId: self.selectedAgentId)
    }
  ...
  func setGlobalWakeWords(_ words: [String]) async {
    ...
    do {
      _ = try await self.operatorGateway.request(
        method: "voicewake.set", paramsJSON: json, timeoutSeconds: 12)
    } catch {
      // Best-effort only.
    }
  }
  ...
  private func handleOperatorGatewayServerEvent(_ evt: EventFrame) async {
    ...
    case "voicewake.changed":
      ...
      guard let decoded = try? GatewayPayloadDecoding.decode(payload, as: Payload.self) else {
        return
      }
      ...
      check: { [weak self] in
        ...
        do {
          let data = try await self.operatorGateway.request(
            ...
            paramsJSON: nil,
            timeoutSeconds: 6)
          guard let decoded = try? JSONDecoder().decode(OpenClawGatewayHealthOK.self, from: data)
          else {
            return false
          }
    ...
    do {
      ...
      case .unknownCommand:
        return BridgeInvokeResponse(
          ...
          ok: false,
          error: OpenClawNodeError(
            code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
  ...
  private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse
  {
    ...
    let params =
      (try? Self.decodeParams(OpenClawLocationGetParams.self, from: req.paramsJSON))
      ?? OpenClawLocationGetParams()
    let desired = params.desiredAccuracy ?? (self.isLocationPreciseEnabled() ? .precise : .balanced)
  ...
  private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
    ...
    case OpenClawCanvasCommand.present.rawValue:
      ...
      let params =
        (try? Self.decodeParams(OpenClawCanvasPresentParams.self, from: req.paramsJSON))
  ...
  private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws
    -> BridgeInvokeResponse
  {
    ...
    case OpenClawCanvasA2UICommand.reset.rawValue:
      ...
      let json = try await self.screen.eval(
        javaScript: """
          (() => {
    ...
    case OpenClawCanvasA2UICommand.push.rawValue, OpenClawCanvasA2UICommand.pushJSONL.rawValue:
      ...
      if command == OpenClawCanvasA2UICommand.pushJSONL.rawValue {
        let params = try Self.decodeParams(
          OpenClawCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
        messages = try OpenClawCanvasA2UIJSONL.decodeMessagesFrotsONL(params.jsonl)
      } else {
        do {
          let params = try Self.decodeParams(
            OpenClawCanvasA2UIPushParams.self, from: req.paramsJSON)
  ...
  private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
    ...
    case OpenClawCameraCommand.snap.rawValue:
      ...
      let params =
        (try? Self.decodeParams(OpenClawCameraSnapParams.self, from: req.paramsJSON))
    ...
    case OpenClawCameraCommand.clip.rawValue:
      let params =
        (try? Self.decodeParams(OpenClawCameraClipParams.self, from: req.paramsJSON))
        ?? OpenClawCameraClipParams()

      let suspended =
        (params.includeAudio ?? true) ? self.voiceWake.suspendForExternalAudioCapture() : false
  ...
  private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws
    -> BridgeInvokeResponse
  {
    let params =
      (try? Self.decodeParams(OpenClawScreenRecordParams.self, from: req.paramsJSON))
      ?? OpenClawScreenRecordParams()
    if let format = params.format, format.lowercased() != "mp4" {
      throw NSError(
        domain: "Screen", code: 30,
        userInfo: [
  ...
  private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
    ...
    if title.isEmpty, body.isEmpty {
      return BridgeInvokeResponse(
        ...
        ok: false,
        error: OpenClawNodeError(
          code: .invalidRequest, message: "INVALID_REQUEST: empty notification"))
    }
    ...
    if case .failure(let error) = addResult {
      return BridgeInvokeResponse(
        ...
        ok: false,
        error: OpenClawNodeError(
          code: .unavailable, message: "NOTIFICATION_FAILED: \(error.message)"))
    }
    return BridgeInvokeResponse(id: req.id, ok: true)
  }
  ...
  private func handleChatPushInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse
  {
    ...
    guard !text.isEmpty else {
      return BridgeInvokeResponse(
        ...
        ok: false,
        error: OpenClawNodeError(
          code: .invalidRequest, message: "INVALID_REQUEST: empty chat.push text"))
    }
  ...
  private func presentNotificationPermissionGuidanceForExecApprovalIfNeeded(approvalId: String)
    async
  {
  ...
  private func runNotificationCall<T: Sendable>(
    ...
    return await withCheckedContinuation {
      (cont: CheckedContinuation<Result<T, NotificationCallError>, Never>) in
  ...
  private func handlePhotosInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
    let params =
      (try? Self.decodeParams(OpenClawPhotosLatestParams.self, from: req.paramsJSON))
  ...
  private func handleContactsInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse
  {
    ...
    case OpenClawContactsCommand.search.rawValue:
      let params =
        (try? Self.decodeParams(OpenClawContactsSearchParams.self, from: req.paramsJSON))
  ...
  private func handleCalendarInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse
  {
    ...
    case OpenClawCalendarCommand.events.rawValue:
      let params =
        (try? Self.decodeParams(OpenClawCalendarEventsParams.self, from: req.paramsJSON))
  ...
  private func handleRemindersInvoke(_ req: BridgeInvokeRequest) async throws
    -> BridgeInvokeResponse
  {
    switch req.command {
    case OpenClawRemindersCommand.list.rawValue:
      let params =
        (try? Self.decodeParams(OpenClawRemindersListParams.self, from: req.paramsJSON))
  ...
  private func handleMotionInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
    ...
    case OpenClawMotionCommand.activity.rawValue:
      let params =
        (try? Self.decodeParams(OpenClawMotionActivityParams.self, from: req.paramsJSON))
    ...
    case OpenClawMotionCommand.pedometer.rawValue:
      let params =
        (try? Self.decodeParams(OpenClawPedometerParams.self, from: req.paramsJSON))
...
extension NodeAppModel {
  var mainSessionKey: String {
    ...
    let defaultId = (self.gatewayDefaultAgentId ?? "").trimmingCharacters(
      in: .whitespacesAndNewlines)
...
extension NodeAppModel {
  ...
  fileprivate nonisolated static func shouldStartOperatorGatewayLoop(
    ...
    let trimmedBootstrapToken =
      bootstrapToken?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
  ...
  ) -> (token: String?, bootstrapToken: String?, password: String?) {
  ...
  private func clearPersistedGatewayBootstrapTokenIfNeeded() {
    ...
    self.activeGatewayConnectConfig = Self.clearingBootstrapToken(
      in: self.activeGatewayConnectConfig)

    let trimmedInstanceId =
      UserDefaults.standard.string(forKey: "node.instanceId")?
      .trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
  ...
  private func startNodeGatewayLoop(
    ...
    self.nodeGatewayTask = Task { [weak self] in
      ...
      while !Task.isCancelled {
        if shouldSkipReconnection() { continue }
        if await isGatewayConnected() { continue }

        await updateUIForConnectionAttempt(attempt)
        do {
          try await attemptConnection(
            ...
            sessionBox: sessionBox,
            attempt: attempt,
            didFallbackClientId: &didFallbackClientId,
            currentOptions: &currentOptions,
            pausedForPairingApproval: &pausedForPairingApproval
          )
          attempt = 0
          try? await Task.sleep(nanoseconds: 1_000_000_000)
        } catch {
          if Task.isCancelled { break }
          attempt = handleConnectionError(
            error: error,
            attempt: attempt,
            stableID: stableID,
            didFallbackClientId: &didFallbackClientId,
      ...
      await finalizeGatewayLoop(pausedForPairingApproval: pausedForPairingApproval)
    }
  }

  private func shouldSkipReconnection() -> Bool {
    if gatewayPairingPaused { return true }
    if !gatewayAutoReconnectEnabled { return true }
    if shouldPauseReconnectLoopInBackground(source: "node_loop") { return true }
    return false
  }

  private func updateUIForConnectionAttempt(_ attempt: Int) async {
    await MainActor.run {
      guard !isLocalGatewayFixtureEnabled else { return }
      gatewayStatusText = attempt == 0 ? "Connecting…" : "Reconnecting…"
  ...
  private func attemptConnection(
    ...
    sessionBox: WebSocketSessionBox?,
    attempt: Int,
    didFallbackClientId: inout Bool,
    currentOptions: inout GatewayConnectOptions,
    pausedForPairingApproval: inout Bool
  ) async throws {
    let epochMs = Int(Date().timeIntervalSince1970 * 1000)
    let reconnectAuth = currentGatewayReconnectAuth(
      fallbackToken: token,
      fallbackBootstrapToken: bootstrapToken,
      fallbackPassword: password
    )
    ...
    try await nodeGateway.connect(
      ...
      onConnected: { [weak self] in
        await self?.handleGatewayConnected(
          url: url, stableID: stableID, reconnectAuth: reconnectAuth, options: connectedOptions,
          sessionBox: sessionBox)
      },
      onDisconnected: { [weak self] reason in await self?.handleGatewayDisconnected(reason: reason)
      },
  ...
  private func handleConnectionError(
    ...
    if !didFallbackClientId, let fallbackClientId = resolveClientIdFallback(
      currentClientId: currentOptions.clientId, error: error
    )
    {
      ...
      await MainActor.run { gatewayStatusText = "Gateway rejected client id. Retrying…" }
      return newAttempt
    }

    newAttempt += 1
    handleConnectionProblem(error: error)
    GatewayDiagnostics.log("gateway connect error: \(error.localizedDescription)")
    
    if let problem = getConnectionProblem(error: error), problem.pauseReconnect {
      return newAttempt
    }

    let sleepSeconds = min(8.0, 0.5 * pow(1.7, Double(newAttempt)))
    try? await Task.sleep(nanoseconds: UInt64(sleepSeconds * 1_000_000_000))
    return newAttempt
  }

  private func finalizeGatewayLoop(pausedForPairingApproval: Bool) async {
    if pausedForPairingApproval { return }
    await MainActor.run {
      guard !isLocalGatewayFixtureEnabled else { return }
      lastGatewayProblem = nil
      ...
      mainSessionBaseKey = "main"
      talkMode.updateMainSessionKey(mainSessionKey)
...
extension NodeAppModel {
  ...
  func runSharePipelineSelfTest() async {
    ...
    guard
      let deepLink = ShareToAgentDeepLink.buildURL(
        from: payload,
  ...
  private func resumePendingForegroundNodeActionsIfNeeded(trigger: String) async {
    ...
    do {
      ...
      self.pendingActionLogger.info(
        "pending actions count=\(decoded.actions.count, privacy: .public)")
  ...
  private func applyPendingForegroundNodeActions(
    ...
    for action in actions {
      ...
      self.pendingActionLogger
        .info(
          "pending replay trigger=\(trigger, privacy: .public) id=\(action.id, privacy: .public)")
  ...
  private func ackPendingForegroundNodeAction(
    ...
    do {
      ...
      self.pendingActionLogger.error(
        "pending ack error=\(String(describing: error), privacy: .public)")
      return false
    }
  }
  ...
  private func handleWatchQuickReply(_ event: WatchQuickReplyEvent) async {
    switch await self.watchReplyCoordinator.ingest(
      event, isGatewayConnected: self.isGatewayConnected())
    {
  ...
  private static func makeWatchExecApprovalItem(from prompt: ExecApprovalPrompt)
  ...
  private func makeWatchChatPreview() async -> WatchChatPreview {
    do {
      ...
      if self.isAppleReviewDemoModeEnabled {
        payload = try await self.appleReviewDemoChatTransport.requestHistory(
  ...
  private func makeWatchAppSnapshot(
    ...
    let watchGatewayStatusText =
      watchGatewayConnected || displayStatusText != "Connected"
      ? displayStatusText
  ...
  private func normalizedWatchChatGatewayStableID(_ event: WatchAppCommandEvent) -> String? {
    let gatewayStableID =
      event.gatewayStableID?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
  ...
  private func forwardWatchChatMessage(
    ...
    let sessionKey =
      (event.sessionKey?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false)
      ? event.sessionKey!
      : self.chatSessionKey
    self.focusChatSession(sessionKey)
  ...
  private nonisolated static func watchExecApprovalIDsNeedingFetch(
    candidateIDs: [String],
    cachedApprovalIDs: [String]
  ) -> [String] {
    let cachedIDs = Set(
      cachedApprovalIDs.compactMap { id -> String? in
  ...
  private func hydrateWatchExecApprovalCacheIfNeeded(reason: String) async {
    ...
    for approvalId in missingApprovalIDs {
      ...
      case .failed(let message):
        ...
        self.watchExecApprovalLogger.error(
          "watch approval hydrate reason=\(reason, privacy: .public)")
        self.watchExecApprovalLogger.error(
          "watch approval hydrate error=\(message, privacy: .public)")
      }
    }
  }
  ...
  private func pendingExecApprovalIDsForWatchRecovery() async -> [String] {
    ...
    for snapshot in delivered {
      guard
        ExecApprovalNotificationBridge.payloadKind(userInfo: snapshot.userInfo)
  ...
  func handleExecApprovalRequestedRemotePush(approvalId: String) async -> Bool {
    ...
    case .failed(let message):
      ...
      self.watchExecApprovalLogger.error(
        "watch approval push fetch error=\(message, privacy: .public)")
      return false
    }
  }
  ...
  func handleExecApprovalResolvedRemotePush(approvalId: String) async {
    ...
    let hadPendingRecoveryID = self.pendingWatchExecApprovalRecoveryIDs.contains(
      normalizedApprovalID)
    let hadGuidancePrompt =
      self.pendingNotificationPermissionGuidancePrompt?.approvalId == normalizedApprovalID
    ...
    if hadApprovalSurface {
      await self.publishWatchExecApprovalExpired(
        approvalId: normalizedApprovalID, reason: .resolved)
    }
    self.clearPendingExecApprovalPromptIfMatches(normalizedApprovalID)
  }
  ...
  func handleSilentPushWake(_ userInfo: [AnyHashable: Any]) async -> Bool {
    ...
    guard Self.isSilentPushPayload(userInfo) else {
      self.pushWakeLogger.info(
        "Ignored APNs payload wakeId=\(wakeId, privacy: .public): not silent push")
      return false
    }
    ...
    let isExecApprovalRequestPush =
      execApprovalPushKind == ExecApprovalNotificationBridge.requestedKind
  ...
  func presentExecApprovalNotificationPrompt(_ prompt: ExecApprovalNotificationPrompt) async {
    ...
    case .failed(let message):
      ...
      self.execApprovalNotificationLogger.error(
        "approval prompt fetch reason=\(message, privacy: .public)")
    }
  }
  ...
  private static func makeExecApprovalPrompt(from details: ExecApprovalGetResponse)
    -> ExecApprovalPrompt?
  {
  ...
  ) async -> ExecApprovalPromptFetchOutcome {
  ...
  private func resolveExecApprovalNotificationDecision(
    ...
    let resolutionReason =
      (normalizedSourceReason?.isEmpty == false) ? normalizedSourceReason! : "direct"
    ...
    do {
      ...
      if Self.isApprovalNotificationStaleError(error) {
        ...
        self.clearPendingExecApprovalPromptIfMatches(normalizedApprovalID)
        await self.publishWatchExecApprovalExpired(
          approvalId: normalizedApprovalID, reason: .notFound)
        return .stale
      }
      if Self.isApprovalNotificationUnavailableError(error) {
        ...
        self.clearPendingExecApprovalPromptIfMatches(normalizedApprovalID)
        await self.publishWatchExecApprovalExpired(
          approvalId: normalizedApprovalID, reason: .unavailable)
        return .unavailable
      }
      let logMessage =
  ...
  private func clearNotificationPermissionGuidancePromptIfMatches(_ approvalId: String) {
    let normalizedApprovalID = approvalId.trimmingCharacters(in: .whitespacesAndNewlines)
    guard self.pendingNotificationPermissionGuidancePrompt?.approvalId == normalizedApprovalID
    else { return }
    self.pendingNotificationPermissionGuidancePrompt = nil
  }
  ...
  private func ensureOperatorApprovalConnectionForWatchReview(timeoutMs: Int, reason: String) async
    -> Bool
  {
    ...
    self.grantBackgroundReconnectLease(
      seconds: leaseSeconds, reason: "watch_review_\(reconnectReason)")
    GatewayDiagnostics.log(
  ...
  private func performBackgroundAliveBeaconIfNeeded(
    wakeId: String,
    trigger: BackgroundAliveBeacon.Trigger
  ) async -> BackgroundAliveWakeAttemptResult {
    let startedAt = Date()
    let makeResult: (Bool, Bool, String) -> BackgroundAliveWakeAttemptResult = {
      applied, handled, reason in
    ...
    guard self.isBackgrounded else {
      self.pushWakeLogger.info(
        "Wake no-op wakeId=\(wakeId, privacy: .public): app not backgrounded")
    ...
    if !gatewayConnected {
      guard let cfg = self.activeGatewayConnectConfig else {
        self.pushWakeLogger.info(
          "Wake no-op wakeId=\(wakeId, privacy: .public): no active gateway config")
      ...
      now: now,
      lastSuccessAtMs: UserDefaults.standard.object(forKey: Self.backgroundAliveLastSuccessAtMsKey)
        as? Double)
    {
  ...
  private func publishBackgroundAliveBeacon(
    trigger: BackgroundAliveBeacon.Trigger
  ) async -> (handled: Bool, reason: String) {
    do {
...
extension NodeAppModel {
  ...
  private func handleAgentDeepLink(_ link: AgentDeepLink, originalURL: URL) async {
    ...
    if message.count > IOSDeepLinkAgentPolicy.maxMessageChars {
      self.screen.errorText =
        "Deep link too large (message exceeds "
  ...
  private func queueAgentDeepLinkPrompt(
    _ prompt: AgentDeepLinkPrompt, initialDelaySeconds: TimeInterval
  ) {
  ...
  private func submitAgentDeepLink(_ link: AgentDeepLink, messageCharCount: Int) async {
    do {
      ...
      self.deepLinkLogger.error(
        "agent deep link send failed: \(error.localizedDescription, privacy: .public)")
    }
  }
  ...
  private func isUnattendedDeepLinkAllowed(_ key: String?) -> Bool {
    ...
    return normalizedKey == Self.canvasUnattendedDeepLinkKey
      || normalizedKey == Self.expectedDeepLinkKey()
  }
...
extension NodeAppModel {
  ...
  extension NodeAppModel {
    ...
    nonisolated static func _test_makeWatchChatItems(from raw: [OpenClawKit.AnyCodable])
      -> [OpenClawWatchChatItem]
    {
    ...
    func _test_applyPendingForegroundNodeActions(
      _ actions: [(id: String, command: String, paramsJSON: String?)]
    ) async {
    ...
    func _test_pendingNotificationPermissionGuidancePrompt()
      -> NotificationPermissionGuidancePrompt?
    {
      self.pendingNotificationPermissionGuidancePrompt
    }
