...
final class WatchChatCoordinator {
  ...
  func removeQueuedCommand(commandId: String, gatewayStableID: String?) {
    ...
    guard
      let index = self.queuedCommands.firstIndex(where: {
  ...
  private func restoreQueue() {
    ...
    self.queuedCommands = persisted.compactMap { queued in
      ...
      guard !owner.isEmpty, !commandId.isEmpty, !text.isEmpty, seenSet.insert(commandId).inserted
      else {
        return nil
      }
      seen.append(commandId)
      return QueuedCommand(
        gatewayStableID: owner, event: self.command(queued.event, taggedFor: owner))
    }
  ...
  private func command(_ event: WatchAppCommandEvent, taggedFor gatewayStableID: String)
