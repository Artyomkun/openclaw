...
struct OnboardingWizardView: View {
  ...
  @State private var statusLine: String =
    "In your OpenClaw chat, run /pair qr, then scan the code here."
  ...
  private static let pairingAutoResumeTicker = Timer.publish(every: 2.0, on: .main, in: .common)
    .autoconnect()
    ...
    Group {
      ...
      if self.issue.needsPairing {
        Section {
          ...
          let commandLine =
            self.currentProblem?.actionCommand ?? "openclaw devices approve <requestId>"
          Text(
