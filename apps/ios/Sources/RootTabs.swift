...
struct RootTabs: View {
  ...
  @State private var selectedSettingsRoute: SettingsRoute? = Self.initialSidebarDestination
    .settingsRoute
  ...
  private static var requestedInitialSidebarDestination: SidebarDestination? {
    ...
    let requested = arguments[valueIndex].trimmingCharacters(in: .whitespacesAndNewlines)
      .lowercased()
    ...
    TabView(selection: self.$selectedTab) {
      ...
      TalkProTab(openSettings: { self.selectSidebarDestination(.gateway) })
        .tabItem {
          Label(
            "Talk",
            systemImage: self.appModel.talkMode.isEnabled
  ...
  private var sidebarSplitContent: some View {
    GeometryReader { proxy in
      ...
      let sidebarWidth = self.sidebarWidth(
        containerWidth: proxy.size.width, isDrawerLayout: isDrawerLayout)
      Group {
        if isDrawerLayout {
  ...
  private func sidebarDestinationButton(
    ...
    .buttonStyle(.plain)
    .foregroundStyle(
      destination == self.selectedSidebarDestination ? OpenClawBrand.accent : .primary
    )
    .listRowBackground(
  ...
  private var sidebarDetail: some View {
    ...
    case .gateway:
      SettingsProTab(
        directRoute: self.selectedSettingsRoute ?? self.selectedSidebarDestination.settingsRoute
          ?? .gateway,
        headerLeadingAction: self.sidebarHeaderLeadingAction,
        onRouteChange: self.handleSettingsRouteChange)
    }
  ...
  private var activeExecApprovalPromptSuppressionID: String? {
    guard self.selectedTab == .settings, self.selectedSettingsRoute == .notifications else {
      return nil
    }
  ...
  private var sidebarHeaderLeadingAction: OpenClawSidebarHeaderAction? {
    guard
      Self.shouldShowSidebarRevealInDestinationHeader(
  ...
  private func updateCanvasDebugStatus() {
    ...
    let title = self.appModel.gatewayDisplayStatusText.trimmingCharacters(
  ...
  private func makeHomeCanvasPayload() -> RootTabsHomeCanvasPayload {
    ...
    case .connected:
      return RootTabsHomeCanvasPayload(
        ...
        footer:
          "OpenClaw only runs phone-side capabilities while the app is connected and permitted.")
    case .connecting:
      return RootTabsHomeCanvasPayload(
...
extension RootTabs {
  ...
  private func hasExistingGatewayConfig() -> Bool {
    ...
    let preferredStableID = self.preferredGatewayStableID.trimmingCharacters(
      in: .whitespacesAndNewlines)
