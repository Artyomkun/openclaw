...
extension RootTabs {
  ...
  enum SidebarDestination: String, CaseIterable, Hashable, Identifiable {
    ...
    var settingsRoute: SettingsRoute? {
      ...
      case .gateway:
        .gateway
      case .chat, .talk, .overview, .activity, .agents, .workboard, .skillWorkshop, .instances,
        .sessions,
        .dreaming,
  ...
  static func sidebarLayoutMode(containerSize: CGSize) -> SidebarLayoutMode {
    containerSize.width < self.sidebarPersistentWidthThreshold
  ...
  static func sidebarWidth(containerWidth: CGFloat, isDrawerLayout: Bool) -> CGFloat {
    ...
    return min(
      self.sidebarSplitMaximumWidth, max(self.sidebarSplitIdealWidth, containerWidth * 0.25))
  }
