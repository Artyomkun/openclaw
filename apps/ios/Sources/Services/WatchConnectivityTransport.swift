...
private struct WatchConnectivityTransportCallbacks {
  ...
  var execApprovalSnapshotRequestHandler:
...
private func sendReachableWatchMessage(_ payload: [String: Any], with session: WCSession)
  async throws
{
...
final class WatchConnectivityTransport: NSObject, @unchecked Sendable {
  private nonisolated static let logger = Logger(
    subsystem: "ai.openclawfoundation.app", category: "watch.messaging")
  ...
  func sendPayload(_ payload: [String: Any]) async throws -> WatchNotificationSendResult {
    ...
    if session.isReachable {
      do {
        ...
        Self.logger.error(
          "watch sendMessage failed: \(error.localizedDescription, privacy: .public)")
      }
    }
