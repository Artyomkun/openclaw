...
private func makeRealtimeAudioTapBlock(
  ...
  onAudio: @escaping (Data, Double, Float) -> Void
) -> AVAudioNodeTapBlock {
  { buffer, _ in
...
private actor RealtimeAudioSender {
  ...
  private static func requestJSON<T: Decodable>(
    ...
    guard let json = String(data: data, encoding: .utf8) else {
      throw NSError(
        domain: "RealtimeTalkRelay", code: 4,
        userInfo: [
...
final class RealtimeTalkRelaySession {
  ...
  func start() async throws {
    ...
    do {
      ...
      guard
        let relaySessionId = result.relaysessionid?.trimmingCharacters(in: .whitespacesAndNewlines),
        !relaySessionId.isEmpty
      else {
        throw NSError(
          domain: "RealtimeTalkRelay", code: 1,
          userInfo: [
      ...
      case .failed(let issue):
        ...
        throw NSError(
          domain: "RealtimeTalkRelay", code: 6,
          userInfo: [
  ...
  private func createRelaySession() async throws -> TalkSessionCreateResult {
    ...
    guard let json = String(data: data, encoding: .utf8) else {
      throw NSError(
        domain: "RealtimeTalkRelay", code: 2,
        userInfo: [
  ...
  private func configureAudioContract(_ raw: AnyCodable?) {
    ...
    {
      let message =
        "unexpected realtime relay audio contract input=\(inputEncoding) output=\(outputEncoding)"
      self.logger.warning("\(message, privacy: .public)")
    }
    self.inputSampleRateHz =
      audio["inputSampleRateHz"]?.doubleValue
      ?? Double(Self.defaultSampleRateHz)
    self.outputSampleRateHz =
      audio["outputSampleRateHz"]?.doubleValue
      ?? Double(Self.defaultSampleRateHz)
  }
  ...
  private func handleGatewayEvent(_ event: EventFrame) async {
    ...
    case "audio":
      ...
      self.markOutputAudioStarted(
        byteCount: data.count, nowMs: ProcessInfo.processInfo.systemUptime * 1000)
  ...
  private func timeoutStartupWaiterIfNeeded() {
    guard !self.isClosed, self.startupWaiter != nil, !self.hasReceivedReady,
      self.startupIssue == nil
    else {
      return
    }
  ...
  private func markOutputAudioStarted(byteCount: Int, nowMs: Double) {
    ...
    self.outputPlaybackExpectedEndMs =
      max(nowMs, self.outputPlaybackExpectedEndMs) + chunkDurationMs
  ...
  private func handleToolCall(_ payload: [String: AnyCodable]) async {
    ...
    do {
      ...
      guard let runId = startResponse.runId ?? startResponse.idempotencyKey else {
        throw NSError(
          domain: "RealtimeTalkRelay", code: 3,
          userInfo: [
  ...
  private func handleAgentControlToolCall(
    ...
    var payload: [String: Any] = [
      "sessionId": relaySessionId,
      "sessionKey": self.options.sessionKey,
      "text": controlArgs["text"]?.stringValue?.trimmingCharacters(in: .whitespacesAndNewlines)
        ?? "status",
    ]
    ...
    let response = try await self.requestJSON(
      ...
      timeoutSeconds: 30)
    let result =
      response.dictionaryValue?.mapValues(\.foundationValue) ?? [
  ...
  private func startMicrophonePump() throws {
    ...
    guard format.sampleRate > 0, format.channelCount > 0 else {
      throw NSError(
        domain: "RealtimeTalkRelay", code: 5,
        userInfo: [
    ...
    let tapBlock = makeRealtimeAudioTapBlock(
      ...
      Task {
        ...
        guard let message = await audioSender.send(encoded, timestampMs: timestampMs) else {
          return
        }
  ...
  private func startPendingOutputPlaybackIfNeeded() {
    ...
    for chunk in chunks {
      self.markOutputAudioStarted(
        byteCount: chunk.count, nowMs: ProcessInfo.processInfo.systemUptime * 1000)
      self.onSpeakingChanged(true)
  ...
  fileprivate nonisolated static func encodePCM16(
    ...
    let outputCount = max(
      1, Int((Double(frameCount) * targetSampleRate / inputSampleRate).rounded(.down)))
