...
final class TalkModeManager: NSObject {
  ...
  func applyAudioRoutePreferenceChanged() {
    ...
    do {
      ...
      GatewayDiagnostics.log(
        "talk audio route preference failed error=\(error.localizedDescription)")
    }
  }
  ...
  private func applyOpenAIRealtimeSelectionDefaults() {
    ...
    self.gatewayTalkUsesRealtimeRelay = true
    self.gatewayTalkTransportLabel = "Gateway Relay"
    self.gatewayTalkRealtimeProviderLabel = Self.displayName(
      forProvider: self.realtimeProvider ?? "openai")
  ...
  func stop() {
    ...
    do {
      ...
      self.logger.warning(
        "audio session deactivate failed: \(error.localizedDescription, privacy: .public)")
    }
  ...
  func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws
    -> OpenClawTalkPTTStopPayload
  {
  ...
  private func processTranscript(_ transcript: String, restartAfter: Bool) async {
    ...
    GatewayDiagnostics.log(
      "talk: process transcript chars=\(transcript.count) restartAfter=\(restartAfter)")
    await reloadConfig()
  ...
  private func startRealtimeIfAvailable() async -> RealtimeStartResult {
    ...
    do {
      ...
      GatewayDiagnostics
        .log(
          "talk realtime: unavailable; falling back to speech pipeline error=\(error.localizedDescription)"
        )
      GatewayDiagnostics.log(
  ...
  private func startRealtimeRelayIfAvailable() async -> RealtimeStartResult {
    ...
    GatewayDiagnostics.log(
      "talk.timeline realtime relay start attempt sessionKey=\(self.mainSessionKey)")
    let startedAt = Self.nowSeconds()
  ...
  private func createRealtimeClientSession(
    ...
    model: String?,
    voice: String?
  ) async throws -> TalkRealtimeClientSession {
    guard let gateway else {
      throw NSError(
        domain: "TalkMode", code: 8,
        userInfo: [
          NSLocalizedDescriptionKey: "Gateway not connected"
        ])
    }
    ...
    let res = try await gateway.request(
      method: "talk.client.create", paramsJSON: json, timeoutSeconds: 12)
    return try JSONDecoder().decode(TalkRealtimeClientSession.self, from: res)
  }

  private func consumePrefetchedRealtimeSession(peekOnly: Bool = false)
    -> TalkRealtimeClientSession?
  {
  ...
  private func sendChat(_ message: String, gateway: GatewayNodeSession) async throws
  ...
  private func fetchLatestAssistantText(gateway: GatewayNodeSession, since: Double? = nil)
    async throws -> String?
  {
    let res = try await gateway.request(
      method: "chat.history",
    ...
    do {
      ...
      if canUseElevenLabs, let voiceId, let apiKey {
        ...
        let requestedOutputFormat =
          (desiredOutputFormat?.isEmpty == false) ? desiredOutputFormat : nil
        ...
        self.logger.warning(
          "tts unavailable; falling back to system voice (missing key or voiceId)")
  ...
  private var isSpeechOutputActive: Bool {
    self.isSpeaking || self.incrementalSpeechActive || self.incrementalSpeechTask != nil
      || !self.incrementalSpeechQueue.isEmpty
  }
  ...
  private func ensureIncrementalPrefetchForUpcomingSegment(context: IncrementalSpeechContext)
    -> Bool
  {
  ...
  private func startIncrementalPrefetch(segment: String, context: IncrementalSpeechContext) {
    ...
    let task = Task { [weak self] in
      let stream = ElevenLabsTTSClient(apiKey: apiKey).streamSynthesize(
        voiceId: voiceId, request: request)
      var chunks: [Data] = []
      do {
  ...
  private func consumeIncrementalPrefetchedAudioIfAvailable(
    ...
    if let chunks = prefetch.chunks, !chunks.isEmpty {
      let prefetched = IncrementalPrefetchedAudio(
        chunks: chunks, outputFormat: prefetch.outputFormat)
      self.incrementalSpeechPrefetch = nil
      return prefetched
    }
    ...
    let prefetched = IncrementalPrefetchedAudio(
      chunks: chunks, outputFormat: completed.outputFormat)
    self.incrementalSpeechPrefetch = nil
    return prefetched
  }
  ...
  private func buildIncrementalSpeechContext(directive: TalkDirective?) async
    -> IncrementalSpeechContext
  {
    let requestedVoice = directive?.voiceId?.trimmingCharacters(in: .whitespacesAndNewlines)
    let resolvedVoice = resolveVoiceAlias(requestedVoice)
    if requestedVoice?.isEmpty == false, resolvedVoice == nil {
  ...
  private mutating func extractSegments(isFinal: Bool) -> [String] {
    ...
    while idx < chars.count {
      ...
      if !inCodeBlock {
        ...
        if Self.isBoundary(currentChar)
          || Self.isSoftBoundary(currentChar, bufferedChars: buffer.count)
        {
          lastBoundary = idx + 1
...
extension TalkModeManager {
  func resolveVoiceAlias(_ value: String?) -> String? {
    ...
    if self.voiceAliases.values.contains(where: {
  ...
  func resolveVoiceId(preferred: String?, apiKey: String) async -> String? {
    ...
    do {
      ...
      self.logger
        .info(
          "default voice selected \(name, privacy: .public) (\(first.voiceId, privacy: .public))")
      return first.voiceId
    } catch {
      self.logger.error(
        "elevenlabs list voices failed: \(error.localizedDescription, privacy: .public)")
      return nil
    }
  }
  ...
  private func applyVoiceModeDescriptor(
    _ descriptor: TalkVoiceModeDescriptor, persistAsConfigured: Bool = false
  ) {
  ...
  func reloadConfig() async {
    ...
    do {
      ...
      self.applyLoadedTalkConfig(
        parsed, redactedFallbackMissingScope: loaded.redactedFallbackMissingScope)
    } catch {
  ...
  private func applyTalkModeDescriptor(
    ...
    let transportLabel =
      usesRealtimeRelay ? "Gateway Relay" : (usesRealtimeConfig ? "Native WebRTC" : "Native")
    ...
    self.gatewayTalkRealtimeProviderLabel = realtimeProvider.map {
      Self.displayName(forProvider: $0)
    }
  ...
  private func applyTalkPermissionState(
      ...
      gatewayOwnedVoiceProvider || self.apiKey == nil
    {
      ...
      self.gatewayTalkPermissionState =
        (self.gatewayTalkApiKeyConfigured || gatewayOwnedVoiceProvider)
        ? .ready
        : .apiKeyMissing
    }
  }
  ...
  static func configureAudioSession() throws {
    ...
    GatewayDiagnostics.log(
      "talk audio: session speakerphone=\(forceSpeaker) \(Self.describeAudioSession())")
  }
...
extension TalkModeManager: TalkRealtimeWebRTCSessionDelegate {
  ...
  func realtimeSession(
    _ session: TalkRealtimeWebRTCSession, didReceiveAssistantTranscript text: String
  ) {
    guard session === self.realtimeSession else { return }
    let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
    guard !trimmed.isEmpty else { return }
