...
final class TalkRealtimeWebRTCSession: NSObject {
  private static let logger = Logger(
    subsystem: "ai.openclawfoundation.app", category: "TalkRealtimeWebRTC")
  ...
  func start(
    ...
    model: String?,
    voice: String?,
    prefetchedSession: TalkRealtimeClientSession? = nil
  ) async throws {
    ...
    guard session.isWebRTC else {
      throw NSError(
        domain: "TalkRealtimeWebRTC", code: 1,
        userInfo: [
          NSLocalizedDescriptionKey:
            "Realtime provider returned unsupported transport \(session.transport)"
        ])
    }
    ...
    guard let peer = factory.peerConnection(with: config, constraints: constraints, delegate: self)
    else {
      throw NSError(
        domain: "TalkRealtimeWebRTC", code: 2,
        userInfo: [
  ...
  private func trace(_ message: String) {
    ...
    Self.logger.info(
      "timeline +\(self.elapsedMs(), privacy: .public)ms \(message, privacy: .public)")
  }
    ...
    return try await withCheckedThrowingContinuation { continuation in
      peer.offer(for: constraints) { offer, error in
        if let error {
          ...
          continuation.resume(
            throwing: NSError(
              domain: "TalkRealtimeWebRTC", code: 3,
              userInfo: [
  ...
  private func setLocalDescription(_ description: RTCSessionDescription, peer: RTCPeerConnection)
    async throws
  {
  ...
  private func setRemoteDescription(_ description: RTCSessionDescription, peer: RTCPeerConnection)
    async throws
  {
    try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
  ...
  private func exchangeOffer(_ sdp: String, session: TalkRealtimeClientSession) async throws
    -> String
  {
    let rawURL = session.offerUrl ?? Self.defaultOfferURL
    guard let url = URL(string: rawURL) else {
      throw NSError(
        domain: "TalkRealtimeWebRTC", code: 4,
        userInfo: [
    ...
    guard let http = response as? HTTPURLResponse else {
      throw NSError(
        domain: "TalkRealtimeWebRTC", code: 5,
        userInfo: [
    ...
    let elapsed = Int((ProcessInfo.processInfo.systemUptime - startedAt) * 1000)
    self.trace(
      "openai webrtc offer exchange response status=\(http.statusCode) elapsedMs=\(elapsed)")
    guard (200..<300).contains(http.statusCode) else {
      let body = String(data: data, encoding: .utf8) ?? ""
      throw NSError(
        domain: "TalkRealtimeWebRTC", code: http.statusCode,
        userInfo: [
    ...
    guard let answer = String(data: data, encoding: .utf8),
      ...
      throw NSError(
        domain: "TalkRealtimeWebRTC", code: 6,
        userInfo: [
  ...
  private func handleRealtimeAudioStateEvent(_ event: TalkRealtimeServerEvent) -> Bool {
    ...
    case "response.created":
      ...
      self.markAssistantAudioActive()
      return true
    case "response.audio.done", "response.output_audio.done", "conversation.output_audio.done",
      "response.done":
  ...
  private func bufferToolMetadata(_ event: TalkRealtimeServerEvent) {
    guard Self.isSupportedToolName(event.resolvedName), let key = toolBufferKey(for: event) else {
      return
    }
  ...
  private func handleToolDone(_ event: TalkRealtimeServerEvent) {
    ...
    self.trace(
      "tool call ready name=\(name ?? "unknown") callId=\(callId) argsBytes=\((args ?? "").utf8.count)"
    )
    self.assistantAudioActive = false
    self.assistantAudioFinishTask?.cancel()
    self.assistantAudioFinishTask = nil
  ...
  private func submitConsultToolCall(callId: String, argsJSON: String) async {
    ...
    do {
      ...
      guard let json = String(data: data, encoding: .utf8) else {
        throw NSError(
          domain: "TalkRealtimeWebRTC", code: 7,
          userInfo: [
      ...
      guard let runId = response.runId ?? response.idempotencyKey else {
        throw NSError(
          domain: "TalkRealtimeWebRTC", code: 8,
          userInfo: [
            NSLocalizedDescriptionKey: "Gateway did not return a realtime tool run id"
          ])
      }
      self.trace(
        "tool call gateway request done callId=\(callId) runId=\(runId) elapsedMs=\(requestElapsed)"
      )
      ...
      self.trace(
        "tool call chat result ready callId=\(callId) runId=\(runId) chars=\(result.count)")
      ...
      if Task.isCancelled || self.stopped { return }
      Self.logger.error(
        "realtime tool call failed: \(error.localizedDescription, privacy: .public)")
  ...
  private func submitControlToolCall(callId: String, argsJSON: String) async {
    ...
    do {
      ...
      guard let json = String(data: data, encoding: .utf8) else {
        throw NSError(
          domain: "TalkRealtimeWebRTC", code: 19,
          userInfo: [
      ...
      self.trace(
        "control tool gateway request done callId=\(callId) messageBytes=\(message.utf8.count)")
      ...
      if Task.isCancelled || self.stopped { return }
      Self.logger.error(
        "realtime control tool failed: \(error.localizedDescription, privacy: .public)")
  ...
  private func waitForChatResult(
    ...
    return try await withThrowingTaskGroup(of: String.self) { group in
      group.addTask { [runId, currentSessionKey] in
        for await evt in stream {
          ...
          if chatEvent.state == "final" {
            return OpenClawChatEventText.assistantText(from: chatEvent)
              ?? "OpenClaw finished with no text."
          }
          if chatEvent.state == "aborted" {
            throw NSError(
              domain: "TalkRealtimeWebRTC", code: 9,
              userInfo: [
          ...
          if chatEvent.state == "error" {
            throw NSError(
              domain: "TalkRealtimeWebRTC", code: 10,
              userInfo: [
        ...
        throw NSError(
          domain: "TalkRealtimeWebRTC", code: 11,
          userInfo: [
      ...
      group.addTask {
        ...
        throw NSError(
          domain: "TalkRealtimeWebRTC", code: 12,
          userInfo: [
      ...
      guard let result = try await group.next() else {
        throw NSError(
          domain: "TalkRealtimeWebRTC", code: 13,
          userInfo: [
  ...
  private static func waitForAgentResult(
    ...
    throw NSError(
      domain: "TalkRealtimeWebRTC", code: 16,
      userInfo: [
  ...
  private static func agentWait(
    ...
    runId: String,
    timeoutSeconds: Int
  ) async throws -> AgentWaitResponse {
    ...
    guard let json = String(data: data, encoding: .utf8) else {
      throw NSError(
        domain: "TalkRealtimeWebRTC", code: 17,
        userInfo: [
  ...
  private static func latestAssistantTextFromHistory(
    ...
    guard let json = String(data: data, encoding: .utf8) else {
      throw NSError(
        domain: "TalkRealtimeWebRTC", code: 18,
        userInfo: [
          NSLocalizedDescriptionKey: "Failed to encode OpenClaw history request"
        ])
    }
    let response = try await gateway.request(
      method: "chat.history", paramsJSON: json, timeoutSeconds: 15)
  ...
  nonisolated func dataChannel(_: RTCDataChannel, didReceiveMessageWith buffer: RTCDataBuffer) {
    ...
    Task { @MainActor in
      ...
      do {
        ...
        Self.logger
          .debug(
            "ignored realtime event decode failure: \(error.localizedDescription, privacy: .public)"
          )
      }
    }
  }
