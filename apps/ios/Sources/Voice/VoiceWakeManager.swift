...
private func makeAudioTapEnqueueCallback(queue: AudioBufferQueue)
...
final class VoiceWakeManager: NSObject {
  ...
  private func startRecognition() throws {
    ...
    self.tapQueue = queue
    let tapBlock: @Sendable (AVAudioPCMBuffer, AVAudioTime) -> Void = makeAudioTapEnqueueCallback(
      queue: queue)
    ...
    self.recognitionTask = self.speechRecognizer?.recognitionTask(
      with: request, resultHandler: handler)
    ...
    { [weak self] result, error in
      ...
      Task { @MainActor in
        self?.handleRecognitionCallback(
          transcript: transcript, segments: segments, errorText: errorText)
      }
    }
  }

  private func handleRecognitionCallback(
    transcript: String?, segments: [WakeWordSegment], errorText: String?
  ) {
    if let errorText {
  ...
  private nonisolated static func requestPermissionWithTimeout(
    _ operation: @escaping @Sendable (@escaping @Sendable (Bool) -> Void) -> Void
  ) async -> Bool {
    do {
  ...
  private nonisolated static func deniedByDefaultPermissionMessage(
    kind: String, isUndetermined: Bool
  ) -> String {
    if isUndetermined {
      return "\(kind) permission not granted"
    }
