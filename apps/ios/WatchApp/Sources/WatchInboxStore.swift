...
@MainActor @Observable final class WatchInboxStore {
  ...
  func makeAppCommand(_ command: WatchAppCommand, text: String? = nil) -> WatchAppCommandMessage {
    let snapshotSessionKey = self.appSnapshot?.sessionKey.trimmingCharacters(
  ...
  var hasGatewayTaggedAppSnapshot: Bool {
    let gatewayStableID =
      self.appSnapshot?.gatewayStableID?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
  ...
  private func deliveryKey(messageID: String?, title: String, body: String, sentAtMs: Int?)
    -> String
  {
  ...
  private func postLocalNotification(identifier: String, title: String, body: String, risk: String?)
    async
  {
