    ...
    WatchFaceScroll {
      ...
      if self.inboxHasItems {
        ...
        WatchHeroCard(
          label: "Clear",
          title: "Caught up",
          subtitle: self.store.hasAppSnapshot
            ? "No chats or approvals need you" : "Waiting for iPhone sync",
          accessory: "Ready")
      }
...
private enum WatchNativeTextInput {
  ...
  static func present(
    ...
    WKApplication.shared().visibleInterfaceController?.presentTextInputController(
      ...
      guard
        let text = results?.compactMap(stringValue).first?
...
private struct WatchExecApprovalDetailView: View {
  ...
  var body: some View {
    WatchDetailScroll(title: "Approval") {
      WatchHeroCard(
        label: self.riskText(self.currentRecord?.approval.risk ?? self.record.approval.risk)
          ?? "Review",
        title: self.currentRecord?.approval.commandText ?? self.record.approval.commandText,
        subtitle: self.metadataSummary,
        accessory:
          Self
          .expiresText(self.currentRecord?.approval.expiresAtMs ?? self.record.approval.expiresAtMs)
          ?? "Now")
