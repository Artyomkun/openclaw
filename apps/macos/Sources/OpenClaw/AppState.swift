...
final class AppState {
  ...
  var isPaused: Bool {
    didSet {
      self.ifNotPreview { UserDefaults.standard.set(self.isPaused, forKey: pauseDefaultsKey) }
    }
  }
  ...
  var onboardingSeen: Bool {
    didSet {
      self.ifNotPreview {
        UserDefaults.standard.set(self.onboardingSeen, forKey: onboardingSeenKey)
      }
    }
  }
  ...
  var debugPaneEnabled: Bool {
    didSet {
      self.ifNotPreview {
        UserDefaults.standard.set(self.debugPaneEnabled, forKey: debugPaneEnabledKey)
      }
  ...
  var voiceWakeTriggerChime: VoiceWakeChime {
    didSet {
      self.ifNotPreview {
        self.storeChime(self.voiceWakeTriggerChime, key: voiceWakeTriggerChimeKey)
      }
    }
  }
  ...
  var voiceWakeSendChime: VoiceWakeChime {
    didSet {
      self.ifNotPreview { self.storeChime(self.voiceWakeSendChime, key: voiceWakeSendChimeKey) }
    }
  }
  ...
  var voiceWakeMicName: String {
    didSet {
      self.ifNotPreview {
        UserDefaults.standard.set(self.voiceWakeMicName, forKey: voiceWakeMicNameKey)
      }
    }
  }
  ...
  var voiceWakeTriggersTalkMode: Bool {
    didSet {
      self.ifNotPreview {
        UserDefaults.standard.set(
          self.voiceWakeTriggersTalkMode, forKey: voiceWakeTriggersTalkModeKey)
        if self.swabbleEnabled {
          Task { await VoiceWakeRuntime.shared.refresh(state: self) }
        }
      }
  ...
  var talkShiftToStopEnabled: Bool {
    didSet {
      self.ifNotPreview {
        ...
        Task {
          TalkSpeechInterruptMonitor.shared.setEnabled(
            self.talkShiftToStopEnabled && self.talkEnabled)
        }
      }
    }
  }
  ...
  var iconOverride: IconOverrideSelection {
    didSet {
      self.ifNotPreview {
        UserDefaults.standard.set(self.iconOverride.rawValue, forKey: iconOverrideKey)
      }
    }
  }
  ...
  var canvasEnabled: Bool {
    didSet {
      self.ifNotPreview { UserDefaults.standard.set(self.canvasEnabled, forKey: canvasEnabledKey) }
    }
  }
  ...
  var remoteProjectRoot: String {
    didSet {
      self.ifNotPreview {
        UserDefaults.standard.set(self.remoteProjectRoot, forKey: remoteProjectRootKey)
      }
    }
  }
  ...
  var remoteCliPath: String {
    didSet {
      self.ifNotPreview { UserDefaults.standard.set(self.remoteCliPath, forKey: remoteCliPathKey) }
    }
  }
  ...
  init(preview: Bool = false) {
    ...
    self.voiceWakeSendChime = Self.loadChime(
      ...
      fallback: .system(name: "Glass"))
    if let storedIconAnimations = UserDefaults.standard.object(forKey: iconAnimationsEnabledKey)
      as? Bool
    {
    ...
    self.voiceWakeLocaleID =
      UserDefaults.standard.string(forKey: voiceWakeLocaleKey) ?? Locale.current.identifier
    self.voiceWakeAdditionalLocaleIDs =
      UserDefaults.standard
    ...
    if let storedPhaseSounds = UserDefaults.standard.object(forKey: talkPhaseSoundsEnabledKey)
      as? Bool
    {
    ...
    if let storedShiftToStop = UserDefaults.standard.object(forKey: talkShiftToStopEnabledKey)
      as? Bool
    {
    ...
    let configRemoteUrl =
      configRemoteResolution.directURL?.absoluteString
    ...
    let configRemoteTarget =
      (configRemote?["sshTarget"] as? String)?
    ...
    self.remoteProjectRoot =
      UserDefaults.standard.string(forKey: remoteProjectRootKey)?.nonEmpty ?? ""
    ...
    self.execApprovalMode = ExecApprovalQuickMode.from(
      security: execDefaults.security, ask: execDefaults.ask)
    self.peekabooBridgeEnabled =
      UserDefaults.standard
  ...
  private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?)
    -> String
  {
  ...
  private func applyRemoteTokenState(_ tokenValue: GatewayRemoteConfig.TokenValue) {
    ...
    guard
      self.remoteToken != nextToken || self.remoteTokenDirty
  ...
  private static func updatedRemoteGatewayConfig(
    current: [String: Any],
    draft: RemoteGatewayConfigDraft
    ...
    case .ssh:
      ...
      let expectedRemoteHost =
        CommandResolver.parseSSHTarget(sanitizedTarget)?.host ?? draft.remoteHost
      ...
      changed =
        Self.updateGatewayString(&remote, key: "sshTarget", value: sanitizedTarget) || changed
      changed =
        Self.updateGatewayString(&remote, key: "sshIdentity", value: draft.remoteIdentity)
        || changed
    }
