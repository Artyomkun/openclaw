...
final class CanvasManager {
  ...
  func show(sessionKey: String, path: String? = nil, placement: CanvasPlacement? = nil) throws
    -> String
  {
  ...
  func showDetailed(
    ...
    Self.logger.debug(
      "showDetailed showCanvas effectiveTarget=\(effectiveTarget, privacy: .public)")
  ...
  func snapshot(sessionKey: String, outPath: String?) async throws -> String {
    ...
    guard let controller = self.panelController else {
      throw NSError(
        domain: "Canvas", code: 21, userInfo: [NSLocalizedDescriptionKey: "canvas not available"])
    }
  ...
  private func handleGatewayPush(_ push: GatewayPush) {
    guard case .snapshot(let snapshot) = push else { return }
    let raw =
