...
extension ChannelsSettings {
  ...
  private func configuredChannelTint(
    configured: Bool, running: Bool, hasError: Bool, probeOk: Bool?
  ) -> Color {
  ...
  var whatsAppTint: Color {
    guard
      let status = self.channelStatus("whatsapp", as: ChannelsStatusSnapshot.WhatsAppStatus.self)
  ...
  var telegramTint: Color {
    guard
      let status = self.channelStatus("telegram", as: ChannelsStatusSnapshot.TelegramStatus.self)
    else { return .secondary }
  ...
  var googlechatTint: Color {
    guard
      let status = self.channelStatus(
        "googlechat", as: ChannelsStatusSnapshot.GoogleChatStatus.self)
    else { return .secondary }
    return self.configuredChannelTint(
      configured: status.configured,
      running: status.running,
  ...
  var imessageTint: Color {
    guard
      let status = self.channelStatus("imessage", as: ChannelsStatusSnapshot.IMessageStatus.self)
    else { return .secondary }
    return self.configuredChannelTint(
      configured: status.configured,
      running: status.running,
  ...
  func channelEnabled(_ channel: ChannelItem) -> Bool {
    ...
    let accountActive =
      self.store.snapshot?.channelAccounts[channel.id]?.contains(
  ...
  func channelLastCheck(_ channel: ChannelItem) -> Date? {
    ...
    case "signal":
      return
        self
        .date(
          fromMs: self.channelStatus("signal", as: ChannelsStatusSnapshot.SignalStatus.self)?
            .lastProbeAt)
    case "imessage":
      return
        self
  ...
  func channelHasError(_ channel: ChannelItem) -> Bool {
    ...
    case "discord":
      guard
        let status = self.channelStatus("discord", as: ChannelsStatusSnapshot.DiscordStatus.self)
      else { return false }
      return status.lastError?.isEmpty == false || status.probe?.ok == false
    case "googlechat":
      guard
