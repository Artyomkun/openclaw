...
enum CommandResolver {
  ...
  static func gatewayEntrypoint(in root: URL) -> String? {
    ...
    let openclawEntry = root.appendingPathComponent("openclaw.ts").path
  ...
  static func runtimeResolution(searchPaths: [String]?) -> Result<
    RuntimeResolution, RuntimeResolutionError
  > {
  ...
  private static func nodeManagerBinPaths(home: URL) -> [String] {
    ...
    // fnm
    bins.append(
      contentsOf: self.versionedNodeBinPaths(
  ...
  static func projectOpenClawExecutable(projectRoot: URL? = nil) -> String? {
      ...
      let candidate = root.appendingPathComponent("node_modules/.bin").appendingPathComponent(
        self.helperName
      ).path
  ...
  static func nodeCliPath() -> String? {
    ...
    let candidates = [
      root.appendingPathComponent("openclaw.ts").path,
  ...
  static func openclawNodeCommand(
    ...
    case .success:
      let missingEntry = """
        openclaw entrypoint missing (looked for dist/index.js or openclaw.ts); run pnpm build.
        """
  ...
  private static func sshNodeCommand(
    subcommand: String, extraArgs: [String], settings: RemoteSettings
  ) -> [String]? {
    ...
    let scriptBody = """
      ...
      elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/openclaw.ts" ]; then
        if command -v node >/dev/null 2>&1; then
          CLI="node $PRJ/openclaw.ts"
          node "$PRJ/openclaw.ts" \(quotedArgs);
        else
          echo "Node >=22 required on remote host"; exit 127;
        fi
  ...
  static func connectionSettings(
    ...
    let target =
      defaults.string(forKey: remoteTargetKey)?.nonEmpty
  ...
  private static func sanitizedTarget(_ raw: String) -> String {
    ...
    if trimmed.hasPrefix("ssh ") {
      return trimmed.replacingOccurrences(of: "ssh ", with: "").trimmingCharacters(
  ...
  static func parseSSHTarget(_ target: String) -> SSHParsedTarget? {
    ...
    if trimmed.rangeOfCharacter(from: CharacterSet.whitespacesAndNewlines.union(.controlCharacters))
      != nil
    {
      return nil
    }
