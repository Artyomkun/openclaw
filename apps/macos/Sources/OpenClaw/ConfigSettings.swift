...
extension ConfigSettings {
  ...
  private var actionRow: some View {
    HStack(spacing: 10) {
      ...
      .disabled(
        self.isNixMode || self.store.isSavingConfig || !self.store.configLoaded
          || !self.store
            .configDirty
      )
    }
  ...
  private func sidebarSection(_ section: ConfigSection) -> some View {
    let isExpanded =
      self.activePath == section.path || self.activePath?.hasPrefix("\(section.path).") == true
  ...
  private func sortLookupChildren(_ lhs: ConfigSchemaLookupChild, _ rhs: ConfigSchemaLookupChild)
    -> Bool
  {
