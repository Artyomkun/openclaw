...
final class ControlChannel {
  ...
  func configure(mode: Mode = .local) async throws {
    ...
    case .remote(let target, let identity):
      do {
        _ = (target, identity)
  ...
  private func friendlyGatewayMessage(_ error: Error) -> String {
    ...
    let detail =
      nsError.localizedDescription.isEmpty ? "unknown gateway error" : nsError.localizedDescription
  ...
  private func scheduleRecovery(reason: String) {
    ...
    self.recoveryTask = Task { [weak self] in
      ...
      self.logger.info(
        "control channel recovery starting " + "mode=\(String(describing: mode), privacy: .public) "
      ...
      if mode == .remote {
        do {
          ...
          self.logger.info(
            "control channel recovery ensured SSH tunnel port=\(port, privacy: .public)")
        } catch {
          self.logger.error(
      ...
      } else if case .degraded(let message) = self.state {
  ...
  private static func bridgeToProtocolArgs(
    ...
    if let dict = value.value as? [String: OpenClawKit.AnyCodable],
      ...
      let decoded = try? JSONDecoder().decode(
        [String: OpenClawProtocol.AnyCodable].self, from: data)
    {
      return decoded
    }
