...
extension CritterStatusLabel {
  ...
  private var iconImage: Image {
    let badge: CritterIconRenderer.Badge? =
      if let prominence = self.iconState.badgeProminence, !self.isPaused {
    ...
    if self.isSleeping {
      return Image(
        nsImage: CritterIconRenderer.makeIcon(blink: 1, eyesClosedLines: true, badge: nil))
    }

    return Image(
      nsImage: CritterIconRenderer.makeIcon(
