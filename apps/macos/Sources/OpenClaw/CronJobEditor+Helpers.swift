...
extension CronJobEditor {
  ...
  func hydrateFromJob() {
    ...
    case .agentTurn(let message, let thinking, let timeoutSeconds, _, _, _, _):
  ...
  static func parseDurationMs(_ input: String) -> Int? {
    ...
    let rx = try? NSRegularExpression(
      pattern: "^(\\d+(?:\\.\\d+)?)(ms|s|m|h|d)$", options: [.caseInsensitive])
    guard let match = rx?.firstMatch(in: raw, range: NSRange(location: 0, length: raw.utf16.count))
    else {
      return nil
    }
  ...
  var effectiveSessionTargetRaw: String {
    if self.sessionTarget == .isolated,
      let preserved = self.preservedSessionTargetRaw?.trimmingCharacters(
