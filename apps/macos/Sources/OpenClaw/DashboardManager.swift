...
final class DashboardManager {
  ...
  func show() async throws {
    ...
    dashboardManagerLogger.info(
      "dashboard show requested mode=\(String(describing: mode), privacy: .public)")
    let config = try await self.dashboardConfig(mode: mode)
    dashboardManagerLogger.info(
      "dashboard config url=\(config.url.absoluteString, privacy: .public)")
    ...
    if let controller {
      dashboardManagerLogger.info(
        "dashboard reuse window url=\(url.absoluteString, privacy: .public)")
      controller.show(url: url, auth: auth)
      return
    }

    dashboardManagerLogger.info(
      "dashboard create window url=\(url.absoluteString, privacy: .public)")
    let controller = DashboardWindowController(url: url, auth: auth)
    self.controller = controller
    controller.show(url: url, auth: auth)
  ...
  private func dashboardConfig(mode: AppState.ConnectionMode) async throws
