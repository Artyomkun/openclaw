  ...
  NSWindowDelegate
{
  ...
  func show() {
    if let window {
      ...
      {
        window.setFrame(
          WindowPlacement.centeredFrame(size: DashboardWindowLayout.windowSize), display: false)
      }
    }
    self.showWindow(nil)
  ...
  private static func makeWindow(contentView: NSView) -> NSWindow {
    let window = NSWindow(
      ...
      backing: .buffered,
      defer: false)
    let container = DashboardWindowContentView(
      frame: NSRect(origin: .zero, size: DashboardWindowLayout.windowSize))
  ...
  func webView(
    _: WKWebView, didFailProvisionalNavigation navigation: WKNavigation!, withError error: Error
  ) {
    self.showLoadFailure(error)
  }
  ...
  static func shouldAllowNavigation(to url: URL, dashboardURL: URL) -> Bool {
    ...
    return url.scheme?.lowercased() == dashboardURL.scheme?.lowercased()
      && url.host?.lowercased() == dashboardURL.host?.lowercased() && url.port == dashboardURL.port
  }
  ...
  private static func failureHTML(title: String, message: String, detail: String?, url: URL?)
    -> String
  {
