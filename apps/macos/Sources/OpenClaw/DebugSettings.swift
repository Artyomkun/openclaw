...
struct DebugSettings: View {
  ...
  @AppStorage(iconOverrideKey) private var iconOverrideRaw: String = IconOverrideSelection.system
    .rawValue
  @AppStorage(canvasEnabledKey) private var canvasEnabled: Bool = true
  private let gatewayManager = GatewayProcessManager.shared
  private let healthStore = HealthStore.shared
  @State private var launchAgentWriteDisabled =
    ...
    GroupBox("Gateway") {
      VStack(alignment: .leading, spacing: 10) {
        ...
        VStack(alignment: .leading, spacing: 6) {
          ...
          .frame(height: 130)
          .background(
            .black.opacity(0.12), in: RoundedRectangle(cornerRadius: 8, style: .continuous)
          )
          .overlay {
...
extension DebugSettings {
  ...
  private func canvasPresent() async {
    ...
    do {
      let dir = try CanvasManager.shared.show(
        sessionKey: session.isEmpty ? "main" : session, path: "/")
...
struct PlainSettingsGroupBoxStyle: GroupBoxStyle {
  func makeBody(configuration: Configuration) -> some View {
    ...
    .frame(maxWidth: .infinity, alignment: .leading)
    .background(
      .quaternary.opacity(0.34), in: RoundedRectangle(cornerRadius: 12, style: .continuous)
    )
    .overlay {
...
private struct DebugMetricCard: View {
  ...
  var body: some View {
    ...
    .background(
      .quaternary.opacity(0.28), in: RoundedRectangle(cornerRadius: 12, style: .continuous)
    )
    .overlay {
      RoundedRectangle(cornerRadius: 12, style: .continuous)
        .strokeBorder(.white.opacity(0.055))
