...
enum DeepLinkAgentPolicy {
  ...
  enum ValidationError: Error, Equatable, LocalizedError {
    ...
    var errorDescription: String? {
      switch self {
      case .messageTooLongForConfirmation(let max, let actual):
  ...
  > {
    if !allowUnattended, message.count > self.maxUnkeyedConfirmChars {
      return .failure(
        .messageTooLongForConfirmation(max: self.maxUnkeyedConfirmChars, actual: message.count))
    }
    return .success(())
  }
  ...
  ) -> (deliver: Bool, to: String?, channel: GatewayAgentChannel) {
    if !allowUnattended {
...
final class DeepLinkHandler {
  ...
  func handle(url: URL) async {
    ...
    guard !AppStateStore.shared.isPaused else {
      self.presentAlert(
        title: "OpenClaw is paused", message: "Unpause OpenClaw to run agent actions.")
      return
    }
  ...
  private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
    ...
    if !allowUnattended {
      ...
      if case .failure(let error) = DeepLinkAgentPolicy.validateMessageForHandle(
    ...
    do {
      let effectiveDelivery = DeepLinkAgentPolicy.effectiveDelivery(
        link: link, allowUnattended: allowUnattended)
