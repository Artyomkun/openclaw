...
final class DevicePairingApprovalPrompter {
  ...
  private func loadPendingRequestsFromGateway() async {
    do {
      let list: PairingList = try await GatewayConnection.shared.requestDecoded(
        method: .devicePairList)
      await self.apply(list: list)
    } catch {
      self.logger.error(
        "failed to load device pairing requests: \(error.localizedDescription, privacy: .public)")
    }
  }
  ...
  private func handleAlertResponse(_ response: NSApplication.ModalResponse, request: PendingRequest)
    async
  {
  ...
  private func handle(push: GatewayPush) {
    switch push {
    case .event(let evt) where evt.event == "device.pair.requested":
      ...
      do {
        ...
        self.logger
          .error(
            "failed to decode device pairing request: \(error.localizedDescription, privacy: .public)"
          )
      }
    case .event(let evt) where evt.event == "device.pair.resolved":
      guard let payload = evt.payload else { return }
      do {
  ...
  private func handleResolved(_ resolved: PairingResolvedEvent) {
    let resolution =
      resolved.decision == PairingAlertSupport.PairingResolution.approved.rawValue
  ...
  static func alertSummary(for req: PendingRequest) -> String {
    ...
    return
      "Approve \(subject) to control OpenClaw. Only approve if this is yours; you can remove it later in Settings."
  }
  ...
  private static func makeDetailLine(_ req: PendingRequest) -> NSTextField {
    ...
    if let remoteIp = req.remoteIp?.trimmingCharacters(in: .whitespacesAndNewlines),
      !remoteIp.isEmpty
    {
