...
enum ExecApprovalsStore {
  ...
  static func loadFile() -> ExecApprovalsFile {
    self.withFileLock {
      ...
      do {
        ...
        self.logger.warning(
          "exec approvals load failed: \(error.localizedDescription, privacy: .public)")
        return ExecApprovalsFile(version: 1, socket: nil, defaults: nil, agents: [:])
      }
    }
  }
  ...
  static func saveFile(_ file: ExecApprovalsFile) {
    self.withFileLock {
      do {
        ...
        self.logger.error(
          "exec approvals save failed: \(error.localizedDescription, privacy: .public)")
      }
    }
  }
  ...
  private static func resolveFromFile(_ file: ExecApprovalsFile, agentId: String?)
...
actor SkillBinsCache {
  ...
  func refresh() async {
    do {
      ...
      let trust = Self.buildTrustIndex(
        report: report, searchPaths: CommandResolver.preferredPaths())
