...
final class ExecApprovalsGatewayPrompter {
  ...
  private func handle(push: GatewayPush) async {
    guard case .event(let evt) = push else { return }
    ...
    do {
      ...
      self.logger.error(
        "exec approval handling failed \(error.localizedDescription, privacy: .public)")
    }
  }
  ...
  private func shouldPresent(request: GatewayApprovalRequest) -> PresentationDecision {
    ...
    let activeSession = WebChatManager.shared.activeSessionKey?.trimmingCharacters(
  ...
  private static func fallbackResolution(for request: ExecApprovalPromptRequest)
    -> ExecCommandResolution?
  {
    ...
    let executableName =
      trimmedResolvedPath.map { URL(fileURLWithPath: $0).lastPathComponent } ?? rawExecutable
