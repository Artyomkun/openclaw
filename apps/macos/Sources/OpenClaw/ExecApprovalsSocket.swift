...
enum ExecApprovalsSocketClient {
  ...
  private static func requestDecisionSync(
    ...
    token: String,
    request: ExecApprovalPromptRequest
  ) throws -> ExecApprovalDecision? {
    let fd = socket(AF_UNIX, SOCK_STREAM, 0)
    guard fd >= 0 else {
      throw NSError(
        domain: "ExecApprovals", code: 1,
        userInfo: [
...
enum ExecApprovalsPromptPresenter {
  ...
  static func buildAccessoryView(_ request: ExecApprovalPromptRequest) -> NSView {
    ...
    if let security = request.security?.trimmingCharacters(in: .whitespacesAndNewlines),
      !security.isEmpty
    {
...
private enum ExecHostExecutor {
  ...
  static func handle(_ request: ExecHostRequest) async -> ExecHostResponse {
    ...
    if context.allowlistSatisfied {
      ...
      for (idx, match) in context.allowlistMatches.enumerated() {
        ...
        let resolvedPath =
          idx < context.allowlistResolutions.count
  ...
  private static func ensureScreenRecordingAccess(_ needsScreenRecording: Bool?) async
    -> ExecHostResponse?
  {
...
enum ExecApprovalsSocketPathGuardError: LocalizedError {
  ...
  var errorDescription: String? {
    switch self {
    case .lstatFailed(let path, let code):
      "lstat failed for \(path) (errno \(code))"
    case .parentPathInvalid(let path, let kind):
      "socket parent path invalid (\(kind)) at \(path)"
    case .socketPathInvalid(let path, let kind):
      "socket path invalid (\(kind)) at \(path)"
    case .unlinkFailed(let path, let code):
      "unlink failed for \(path) (errno \(code))"
    case .createParentDirectoryFailed(let path, let message):
      "socket parent directory create failed at \(path): \(message)"
    case .setParentDirectoryPermissionsFailed(let path, let message):
...
private final class ExecApprovalsSocketServer: @unchecked Sendable {
  ...
  func stop() {
    ...
    if !self.socketPath.isEmpty {
      do {
        ...
        self.logger
          .warning(
            "exec approvals socket cleanup failed: \(error.localizedDescription, privacy: .public)")
      }
    }
  }
  ...
  private func openSocket() -> Int32 {
    ...
    do {
      ...
      self.logger
        .error(
          "exec approvals socket path hardening failed: \(error.localizedDescription, privacy: .public)"
        )
      close(fd)
      return -1
    }
  ...
  private func handleClient(fd: Int32) async {
    ...
    do {
      ...
      self.logger.error(
        "exec approvals socket handling failed: \(error.localizedDescription, privacy: .public)")
    }
  }
  ...
  private func handleExecRequest(_ request: ExecHostSocketRequest) async -> ExecHostResponse {
    ...
    let expected = self.hmacHex(
      nonce: request.nonce, ts: request.ts, requestJson: request.requestJson)
