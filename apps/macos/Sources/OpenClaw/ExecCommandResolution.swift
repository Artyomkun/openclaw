...
struct ExecCommandResolution {
  ...
  static func resolveForAllowlist(
    ...
    guard
      let resolution = self.resolveForAllowlistCommand(
        command: command,
  ...
  static func resolve(command: [String], cwd: String?, env: [String: String]?)
    -> ExecCommandResolution?
  {
    let effective = ExecEnvInvocationUnwrapper.unwrapTransparentDispatchWrappersForResolution(
      command)
    guard let raw = effective.first?.trimmingCharacters(in: .whitespacesAndNewlines), !raw.isEmpty
    else {
      return nil
    }
  ...
  private static func resolveExecutable(
    ...
    let expanded =
      rawExecutable.hasPrefix("~")
      ? (rawExecutable as NSString).expandingTildeInPath : rawExecutable
  ...
  private static func unwrapShellMultiplexerInvocation(_ argv: [String]) -> [String]? {
    guard let token0 = argv.first?.trimmingCharacters(in: .whitespacesAndNewlines), !token0.isEmpty
    else {
      return nil
    }
    ...
    var appletIndex = 1
    if appletIndex < argv.count,
      argv[appletIndex].trimmingCharacters(in: .whitespacesAndNewlines) == "--"
    {
      appletIndex += 1
    }
  ...
  private static func splitShellCommandChain(_ command: String) -> [String]? {
    ...
    while idx < chars.count {
      ...
      let lookahead = self.nextShellSignificantCharacter(
        chars: chars, after: idx, inSingle: inSingle)
  ...
  private static func shouldFailClosedForShell(ch: Character, next: Character?, inDouble: Bool)
    -> Bool
  {
