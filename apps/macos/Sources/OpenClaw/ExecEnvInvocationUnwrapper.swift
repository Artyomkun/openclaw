...
enum ExecCommandToken {
  static func basenameLower(_ token: String) -> String {
    ...
    return normalized.split(separator: "/").last.map { String($0).lowercased() }
...
enum ExecEnvInvocationUnwrapper {
  ...
  static func unwrapWithMetadata(_ command: [String]) -> UnwrapResult? {
    ...
    while idx < command.count {
      ...
      if token.hasPrefix("-"), token != "-" {
        ...
        if lower.hasPrefix("-u") || lower.hasPrefix("-c") || lower.hasPrefix("-s")
          || lower.hasPrefix("--unset=") || lower.hasPrefix("--chdir=")
          || lower.hasPrefix("--split-string=") || lower.hasPrefix("--default-signal=")
          || lower.hasPrefix("--ignore-signal=") || lower.hasPrefix("--block-signal=")
        {
          usesModifiers = true
          idx += 1
          continue
  ...
  static func unwrapDispatchWrappersForResolution(_ command: [String]) -> [String] {
    ...
    while depth < self.maxWrapperDepth {
      guard let token = current.first?.trimmingCharacters(in: .whitespacesAndNewlines),
        !token.isEmpty
      else {
        break
      }
