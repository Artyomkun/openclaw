...
enum ExecShellWrapperParser {
  ...
  private static let loginStartupShellNames = Set([
  ...
  private static func startupWrapperRequiresFullArgv(
    ...
    if base0 == "fish", ExecInlineCommandParser.hasFishInitCommandOption(command) {
      return true
    }
    self.loginStartupShellNames.contains(base0)
    ExecInlineCommandParser.hasPosixLoginStartupBeforeInlineCommand(
      command, flags: self.posixInlineFlags)
  ...
  private static func extractCmdInlineCommand(_ command: [String]) -> String? {
    guard
      let idx =
        command
        .firstIndex(where: {
          $0.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "/c"
        })
    else {
      return nil
    }
