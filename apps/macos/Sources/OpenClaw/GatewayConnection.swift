...
private let gatewayConnectionLogger = Logger(
  subsystem: "ai.openclaw", category: "gateway.connection")
...
actor GatewayConnection {
  ...
  private struct LossyCronRunsResponse: Decodable {
    ...
    init(from decoder: Decoder) throws {
      let container = try decoder.container(keyedBy: CodingKeys.self)
      self.entries =
        try container.decodeIfPresent([LossyDecodable<CronRunLogEntry>].self, forKey: .entries)
        ?? []
    }
  }

  init(
    configProvider: @escaping @Sendable () async throws -> Config = GatewayConnection
  ...
  func request(
    ...
    guard let client else {
      throw NSError(
        domain: "Gateway", code: 0, userInfo: [NSLocalizedDescriptionKey: "gateway not configured"])
    }
      ...
      case .local:
          ...
          let fallback = await GatewayEndpointStore.shared.maybeFallbackToTailnet(from: cfg.url)
        {
          await self.configure(
            url: fallback.url, token: fallback.token, password: fallback.password)
          for delayMs in [150, 400, 900] {
            try await Task.sleep(nanoseconds: UInt64(delayMs) * 1_000_000)
            do {
  ...
  private func sessionDefaultString(_ defaults: [String: OpenClawProtocol.AnyCodable]?, key: String)
    -> String
  {
  ...
  func cachedMainSessionKey() -> String? {
    guard let snapshot = self.lastSnapshot else { return nil }
    let trimmed = self.sessionDefaultString(
      snapshot.snapshot.sessiondefaults, key: "mainSessionKey")
    return trimmed.isEmpty ? nil : trimmed
  }
  ...
  private func broadcast(_ push: GatewayPush) {
    if case .snapshot(let snapshot) = push {
  ...
  private func canonicalizeSessionKey(_ raw: String) -> String {
    ...
    let isMainAlias =
      trimmed == "main" || (!mainKey.isEmpty && trimmed == mainKey) || trimmed == mainSessionKey
...
extension GatewayConnection {
  ...
  func setHeartbeatsEnabled(_ enabled: Bool) async -> Bool {
    do {
      ...
      gatewayConnectionLogger.error(
        "setHeartbeatsEnabled failed \(error.localizedDescription, privacy: .public)")
      return false
    }
  }
  ...
  func healthSnapshot(timeoutMs: Double? = nil) async throws -> HealthSnapshot {
    ...
    throw GatewayDecodingError(
      method: Method.health.rawValue, message: "failed to decode health snapshot")
  }
  ...
  func skillsInstall(
    ...
    timeoutMs: Int? = nil
  ) async throws -> SkillInstallResult {
    var params: [String: AnyCodable] = [
  ...
  func sessionsPreview(
    ...
    maxChars: Int? = nil,
    timeoutMs: Int? = nil
  ) async throws -> OpenClawSessionsPreviewPayload {
    let resolvedKeys =
      keys
  ...
  nonisolated static func decodeCronListResponse(_ data: Data) throws -> [CronJob] {
    ...
    if skipped > 0 {
      gatewayConnectionLogger.warning(
        "cron.list skipped \(skipped, privacy: .public) malformed jobs")
    }
    return jobs
  }
  ...
  nonisolated static func decodeCronRunsResponse(_ data: Data) throws -> [CronRunLogEntry] {
    ...
    if skipped > 0 {
      gatewayConnectionLogger.warning(
        "cron.runs skipped \(skipped, privacy: .public) malformed entries")
    }
    return entries
  }
}
