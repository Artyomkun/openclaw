...
actor GatewayEndpointStore {
  ...
  struct Deps {
    ...
    static let live = Deps(
      ...
      localHost: {
        ...
        let tailscaleIP =
          await MainActor.run { TailscaleService.shared.tailscaleIP }
  ...
  init(deps: Deps = .live) {
    ...
    let bind = GatewayEndpointStore.resolveGatewayBindMode(
      ...
      env: ProcessInfo.processInfo.environment)
    let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(
      root: OpenClawConfigFile.loadDict())
  ...
  func requireConfig() async throws -> GatewayConnection.Config {
    await self.refresh()
    switch self.state {
    case .ready(_, let url, let token, let password):
      return (url, token, password)
    case .connecting(let mode, _):
      guard mode == .remote else {
        throw NSError(
          domain: "GatewayEndpoint", code: 1, userInfo: [NSLocalizedDescriptionKey: "Connecting…"])
      }
      return try await self.ensureRemoteConfig(detail: Self.remoteConnectingDetail)
    case .unavailable(let mode, let reason):
      guard mode == .remote else {
        throw NSError(
          domain: "GatewayEndpoint", code: 1, userInfo: [NSLocalizedDescriptionKey: reason])
      }
  ...
  private func setState(_ next: GatewayEndpointState) {
    ...
    switch next {
    case .ready(let mode, let url, _, _):
    ...
    case .connecting(let mode, let detail):
      let modeDesc = String(describing: mode)
      self.logger
        .debug(
  ...
  extension GatewayEndpointStore {
    static func _testResolveGatewayPassword(
      ...
      self.resolveGatewayPassword(
        isRemote: isRemote, root: root, env: env, launchdSnapshot: launchdSnapshot)
    }
