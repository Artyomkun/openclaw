...
struct GatewayEnvironmentStatus: Equatable {
  ...
  static var checking: Self {
    .init(
      kind: .checking, nodeVersion: nil, gatewayVersion: nil, requiredGateway: nil,
      message: "Checking…")
  }
}
...
enum GatewayEnvironment {
  ...
  static func check() -> GatewayEnvironmentStatus {
    ...
    case .success(let runtime):
      ...
      let localPathHint =
        gatewayBin == nil && projectEntrypoint != nil
        ? " (local: \(projectEntrypoint ?? "unknown"))"
        : ""
      let gatewayLabelText =
        gatewayBin != nil
        ? "(\(gatewayLabel))"
      ...
      return GatewayEnvironmentStatus(
        ...
        requiredGateway: expectedString,
        message:
          "Node \(runtime.version.description); gateway \(gatewayVersionText) \(gatewayLabelText)")
    }
  }
  ...
  static func resolveGatewayCommand() -> GatewayCommandResolution {
    ...
    if let entry = projectEntrypoint,
      case .success(let resolvedRuntime) = runtime
    {
      let bind = self.preferredGatewayBind() ?? "loopback"
  ...
  static func installGlobal(version: Semver?, statusHandler: @escaping @Sendable (String) -> Void)
    async
  {
  ...
  static func installGlobal(
    versionString: String?, statusHandler: @escaping @Sendable (String) -> Void
  ) async {
    ...
    let response = await ShellExecutor.runDetailed(
      command: cmd, cwd: nil, env: ["PATH": preferred], timeout: 300)
    if response.success {
  ...
  static func normalizeGatewayVersionOutput(_ raw: String?) -> String? {
    guard var normalized = raw?.trimmingCharacters(in: .whitespacesAndNewlines), !normalized.isEmpty
    else {
      return nil
    }
    ...
    if let parenRange = normalized.range(
      of: #"\s*\([0-9a-fA-F]+\)\s*$"#, options: .regularExpression)
    {
