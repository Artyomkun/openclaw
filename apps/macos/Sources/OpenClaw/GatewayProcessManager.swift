...
final class GatewayProcessManager {
  ...
  enum Status: Equatable {
    ...
    var label: String {
      ...
      case .running(let details):
        ...
        return "Running"
      case .attachedExisting(let details):
    ...
    for attempt in 0..<(hasListener ? 3 : 1) {
      do {
        ...
        if hasListener {
          ...
          self.appendLog(
            "[gateway] existing listener on port \(port) but attach failed: \(reason)\n")
  ...
  private func describe(details instance: String?, port: Int, snap: HealthSnapshot?) -> String {
    ...
    if let snap {
      ...
      let linkId =
        order.first(where: { snap.channels[$0]?.linked == true })
      ...
      let label =
        snap.channelLabels?[linkId] ?? linkId.capitalized
  ...
  private func describeAttachFailure(_ error: Error, port: Int, instance: PortGuardian.Descriptor?)
    -> String
  {
    ...
    if let instance {
      ...
      return
        "Gateway listener found on port \(port) (\(instanceText)) but health check failed: \(message)"
    }
