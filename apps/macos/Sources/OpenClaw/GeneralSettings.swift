...
struct GeneralSettings: View {
  ...
  private var openClawStatusPanel: some View {
    ...
    .padding(.vertical, 14)
    .background(
      .quaternary.opacity(0.45), in: RoundedRectangle(cornerRadius: 12, style: .continuous)
    )
    .overlay {
  ...
  private var connectionStatusIcon: String {
    ...
    case .remote:
      self.state.remoteTransport == .ssh ? "point.3.connected.trianglepath.dotted" : "network"
  ...
  private var connectionStatusTitle: String {
    ...
    case .remote:
      self.state.remoteTransport == .ssh ? "Remote Gateway via SSH" : "Remote Gateway direct"
  ...
  private var connectionStatusSubtitle: String {
    ...
    case .remote:
      let target =
        self.state.remoteTransport == .ssh ? self.state.remoteTarget : self.state.remoteUrl
  ...
  private func advancedTextField(_ title: String, placeholder: String, text: Binding<String>)
    -> some View
  {
    VStack(alignment: .leading, spacing: 4) {
      Text(title)
  ...
  private var remoteSshRow: some View {
    ...
    return VStack(alignment: .leading, spacing: 0) {
      SettingsCardRow(
        title: "SSH target", subtitle: "User and host for the remote Gateway machine."
      ) {
  ...
  private var remoteDirectRow: some View {
    SettingsCardRow(
      title: "Gateway URL", subtitle: "The WebSocket URL exposed by the remote Gateway."
    ) {
  ...
  private var remoteTokenRow: some View {
    VStack(alignment: .leading, spacing: 0) {
      SettingsCardRow(
        ...
        SecureField(
          "remote gateway auth token (gateway.remote.token)", text: self.$state.remoteToken
        )
        .textFieldStyle(.roundedBorder)
  ...
  private var gatewayInstallerCard: some View {
    VStack(alignment: .leading, spacing: 8) {
      ...
      if case .attachedExisting(let details) = self.gatewayManager.status {
...
extension GeneralSettings {
  ...
  func testRemote() async {
    ...
    case .ready(let success):
      self.remoteStatus = .ok(success)
    case .authIssue(let issue):
