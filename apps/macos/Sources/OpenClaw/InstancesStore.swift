...
final class InstancesStore {
  ...
  private func handle(push: GatewayPush) {
    switch push {
    case .event(let evt) where evt.event == "presence":
  ...
  func refresh() async {
    ...
    do {
      ...
      if data.isEmpty {
        ...
        self.lastError = nil
        self.statusMessage =
          "No presence payload from gateway; showing local fallback + health probe."
  ...
  private func probeHealthIfNeeded(reason: String? = nil) async {
    do {
      ...
      let linkLabel =
        linkId.flatMap { snap.channelLabels?[$0] } ?? linkId?.capitalized ?? "channel"
      ...
      self.logger.error(
        "instances health probe failed: \(error.localizedDescription, privacy: .public)")
      if let reason {
        self.statusMessage =
  ...
  func handlePresenceEventPayload(_ payload: OpenClawProtocol.AnyCodable) {
    do {
      ...
      self.logger.error(
        "presence event decode failed: \(error.localizedDescription, privacy: .public)")
  ...
  private func notifyOnNodeLogin(_ instances: [InstanceInfo]) {
    for inst in instances {
      guard let reason = inst.reason?.trimmingCharacters(in: .whitespacesAndNewlines) else {
        continue
      }
