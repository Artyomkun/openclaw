...
struct OpenClawApp: App {
  ...
  var body: some Scene {
    ...
    Window("OpenClaw Settings", id: SettingsWindowOpener.windowID) {
      SettingsRootView(state: self.state, updater: self.delegate.updaterController)
        .frame(
          width: SettingsTab.windowWidth, height: SettingsTab.windowHeight, alignment: .topLeading
        )
  ...
  private func applyStatusItemAppearance(paused _: Bool, sleeping _: Bool) {
    ...
    self.statusItem?.button?.toolTip =
      self.state.voiceWakeMeterActive
  ...
  private var effectiveIconState: IconState {
    ...
    case .workingMain(let kind): return .overridden(kind)
    case .workingOther(let kind): return .overridden(kind)
    case .idle: return .idle
    case .overridden(let kind): return .overridden(kind)
    }
  }
}
  ...
  func applicationDidFinishLaunching(_ notification: Notification) {
    ...
    if let state {
      Task {
        await ConnectionModeCoordinator.shared.apply(
          mode: state.connectionMode, paused: state.isPaused)
      }
    }
    ...
    Task {
      await PeekabooBridgeHostCoordinator.shared.setEnabled(
        AppStateStore.shared.peekabooBridgeEnabled)
    }
