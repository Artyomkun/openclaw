...
struct MenuContent: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 8) {
      Toggle(isOn: self.activeBinding) {
        VStack(alignment: .leading, spacing: 2) {
          ...
          if self.pairingPrompter.pendingCount > 0 {
            ...
            self.statusLine(
              label:
                "Pairing approval pending (\(self.pairingPrompter.pendingCount))\(repairSuffix)",
              color: .orange)
          }
          if self.devicePairingPrompter.pendingCount > 0 {
            ...
            self.statusLine(
              label:
                "Device pairing pending (\(self.devicePairingPrompter.pendingCount))\(repairSuffix)",
              color: .orange)
          }
        }
      }
      ...
      Toggle(
        isOn: Binding(get: { self.state.canvasEnabled }, set: { self.state.canvasEnabled = $0 })
      ) {
      ...
      Button {
        ...
        Label(
          self.state.talkEnabled ? "Stop Talk Mode" : "Talk Mode",
    ...
    .task {
      VoicePushToTalkHotkey.shared.setEnabled(
        voiceWakeSupported && self.state.voicePushToTalkEnabled)
    }
  ...
  private var microphoneMenuItems: some View {
    Group {
      ...
      Button {
        ...
        Label(
          self.defaultMicLabel, systemImage: self.state.voiceWakeMicID.isEmpty ? "checkmark" : ""
        )
        .labelStyle(.titleAndIcon)
      }
      .buttonStyle(.plain)
