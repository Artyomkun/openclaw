...
extension MenuSessionsInjector {
  ...
  private func insertUsageSection(into menu: NSMenu, at cursor: Int, width: CGFloat) -> Int {
      ...
      rows.count > 1
    {
      ...
      item.view = self.makeHostedView(
        rootView: AnyView(
          UsageMenuLabelView(row: primary, width: width, showsChevron: !others.isEmpty)),
        width: width,
        highlighted: true)
      menu.insertItem(item, at: cursor)
      cursor += 1
  ...
  private func makeMessageItem(text: String, symbolName: String, width: CGFloat, maxLines: Int? = 3)
    -> NSMenuItem
  {
    let view = AnyView(
...
extension MenuSessionsInjector {
  ...
  private func refreshCache(force: Bool) async {
    if !force, let updated = self.cacheUpdatedAt,
      Date().timeIntervalSince(updated) < self.refreshIntervalSeconds
    {
      return
    }
...
extension MenuSessionsInjector {
  ...
  private func buildSubmenu(for row: SessionRow, storePath: String) -> NSMenu {
    ...
    let reset = NSMenuItem(
      title: "Reset Session", action: #selector(self.resetSession(_:)), keyEquivalent: "")
    reset.target = self
    ...
    if row.key != self.mainSessionKey, row.key != "global" {
      let del = NSMenuItem(
        title: "Delete Session", action: #selector(self.deleteSession(_:)), keyEquivalent: "")
      del.target = self
  ...
  private func buildThinkingMenu(for row: SessionRow) -> NSMenu {
    ...
    for level in levels {
      ...
      let item = NSMenuItem(
        title: title, action: #selector(self.patchThinking(_:)), keyEquivalent: "")
      item.target = self
  ...
  private func buildVerboseMenu(for row: SessionRow) -> NSMenu {
    ...
    for level in levels {
      let title = level.capitalized
      let item = NSMenuItem(
        title: title, action: #selector(self.patchVerbose(_:)), keyEquivalent: "")
      item.target = self
      item.representedObject = [
        "key": row.key,
        "value": level as Any,
  ...
  private func buildNodeSubmenu(entry: NodeInfo, width: CGFloat) -> NSMenu {
    ...
    menu.addItem(
      self.makeNodeCopyItem(label: "Status", value: NodeMenuEntryFormatter.roleText(entry)))
    ...
    menu.addItem(
      self.makeNodeDetailItem(label: "Connected", value: entry.isConnected ? "Yes" : "No"))
    ...
    if let commands = entry.commands?.filter({
      !$0.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    }),
      !commands.isEmpty
    {
      menu.addItem(
        self.makeNodeMultilineItem(
          label: "Commands",
  ...
  private func makeNodeCopyItem(label: String, value: String) -> NSMenuItem {
    let item = NSMenuItem(
      title: "\(label): \(value)", action: #selector(self.copyNodeValue(_:)), keyEquivalent: "")
    item.target = self
