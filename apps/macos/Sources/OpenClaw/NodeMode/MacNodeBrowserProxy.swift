...
actor MacNodeBrowserProxy {
  ...
  func request(paramsJSON: String?) async throws -> String {
    ...
    guard (200..<300).contains(http.statusCode) else {
      throw NSError(
        domain: "MacNodeBrowserProxy", code: http.statusCode,
        userInfo: [
    ...
    guard let payloadJSON = String(data: payloadData, encoding: .utf8) else {
      throw NSError(
        domain: "MacNodeBrowserProxy", code: 2,
        userInfo: [
  ...
  private static func decodeRequestParams(from raw: String?) throws -> RequestParams {
    guard let raw else {
      throw NSError(
        domain: "MacNodeBrowserProxy", code: 3,
        userInfo: [
  ...
  private static func makeRequest(params: RequestParams, endpoint: Endpoint) throws -> URLRequest {
    let method = (params.method ?? "GET").trimmingCharacters(in: .whitespacesAndNewlines)
      .uppercased()
    let path = (params.path ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
    guard !path.isEmpty else {
      throw NSError(
        domain: "MacNodeBrowserProxy", code: 1,
        userInfo: [
    ...
    guard
      ...
      throw NSError(
        domain: "MacNodeBrowserProxy", code: 4,
        userInfo: [
    ...
    guard let url = components.url else {
      throw NSError(
        domain: "MacNodeBrowserProxy", code: 5,
        userInfo: [
          NSLocalizedDescriptionKey: "INVALID_REQUEST: invalid browser proxy URL"
        ])
    }
  ...
  private static func requireHTTPResponse(_ response: URLResponse) throws -> HTTPURLResponse {
    guard let http = response as? HTTPURLResponse else {
      throw NSError(
        domain: "MacNodeBrowserProxy", code: 6,
        userInfo: [
  ...
  private static func unavailableError(endpoint: Endpoint, cause: Error) -> NSError {
    ...
    let message = """
      ...
      """
    return NSError(
      domain: "MacNodeBrowserProxy", code: 9,
      userInfo: [
  ...
  private static func loadProxyFile(path: String) throws -> ProxyFilePayload {
    ...
    guard values.isRegularFile == true else {
      throw NSError(
        domain: "MacNodeBrowserProxy", code: 7,
        userInfo: [
    ...
    if let fileSize = values.fileSize, fileSize > Self.maxProxyFileBytes {
      throw NSError(
        domain: "MacNodeBrowserProxy", code: 8,
        userInfo: [
