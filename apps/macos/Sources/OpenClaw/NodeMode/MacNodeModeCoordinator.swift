...
final class MacNodeModeCoordinator {
  ...
  nonisolated static func shouldAutoRepairStaleTLSPin(
    url: URL, failure: GatewayTLSValidationFailure
  ) -> Bool {
    guard failure.kind == .pinMismatch else { return false }
    guard url.scheme?.lowercased() == "wss" else { return false }
    guard failure.storeKey == nil || failure.storeKey == self.tlsPinStoreKey(for: url) else {
      return false
    }
    ...
    // A stale leaf pin should not leave the companion app half-connected forever.
  ...
  private func autoRepairStaleTLSPinIfNeeded(error: Error, url: URL?) async -> Bool {
    ...
    guard Self.shouldAutoRepairStaleTLSPin(url: url, failure: tlsError.failure) else {
      return false
    }
    ...
    guard self.autoRepairedTLSFingerprintsByStoreKey[storeKey] != observedFingerprint else {
      return false
    }

    guard GatewayTLSStore.replaceFingerprint(observedFingerprint, stableID: storeKey) else {
      return false
    }
  ...
  private func buildSessionBox(url: URL, connectionMode: AppState.ConnectionMode)
    -> WebSocketSessionBox?
  {
