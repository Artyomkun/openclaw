...
actor MacNodeRuntime {
    ...
    do {
      ...
      default:
        return Self.errorResponse(
          req, code: .invalidRequest, message: "INVALID_REQUEST: unknown command")
      }
    } catch {
  ...
  private func handleBrowserProxyInvoke(_ req: BridgeInvokeRequest) async throws
  ...
  private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse
  {
    ...
    let desired = params.desiredAccuracy ?? (Self.locationPreciseEnabled() ? .precise : .balanced)
  ...
  {
    let params =
      (try? Self.decodeParams(MacNodeScreenRecordParams.self, from: req.paramsJSON))
  ...
  private func handleScreenSnapshotInvoke(_ req: BridgeInvokeRequest) async throws
    -> BridgeInvokeResponse
  {
    ...
    do {
      ...
      case .invalidScreenIndex(let idx):
        return Self.errorResponse(
          req,
          code: .invalidRequest,
  ...
  private func handleA2UIReset(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
    ...
    let sessionKey = self.mainSessionKey
    let json = try await CanvasManager.shared.eval(
      sessionKey: sessionKey,
      javaScript: """
        (() => {
  ...
  private func ensureA2UIHost() async throws {
    ...
    guard let a2uiUrl = await self.resolveA2UIHostUrlWithCapabilityRefresh() else {
      throw NSError(
        domain: "Canvas", code: 30,
        userInfo: [
          NSLocalizedDescriptionKey:
            "A2UI_HOST_NOT_CONFIGURED: gateway did not advertise canvas host"
        ])
    }
    let sessionKey = self.mainSessionKey
  ...
  private func isA2UIReady(poll: Bool = false) async -> Bool {
    ...
    while true {
      do {
        let sessionKey = self.mainSessionKey
        let ready = try await CanvasManager.shared.eval(
          sessionKey: sessionKey,
          javaScript: """
            (() => {
              const host = globalThis.openclawA2UI;
  ...
  private func handleSystemRun(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
    ...
    guard !command.isEmpty else {
      return Self.errorResponse(
        req, code: .invalidRequest, message: "INVALID_REQUEST: command required")
    }
    let sessionKey =
      (params.sessionKey?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false)
    ...
    if !envOverrideDiagnostics.blockedKeys.isEmpty || !envOverrideDiagnostics.invalidKeys.isEmpty {
      ...
      if !envOverrideDiagnostics.blockedKeys.isEmpty {
        details.append(
          "blocked override keys: \(envOverrideDiagnostics.blockedKeys.joined(separator: ", "))")
      }
      ...
      return Self.errorResponse(
        ...
        code: .invalidRequest,
        message:
          "SYSTEM_RUN_DENIED: environment override rejected (\(details.joined(separator: "; ")))")
    }
    ...
    if evaluation.security == .allowlist, !evaluation.allowlistSatisfied, !evaluation.skillAllow,
      !approvedByAsk
    {
      await self.emitExecEvent(
        "exec.denied",
  ...
  private func handleSystemWhich(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
    ...
    guard !bins.isEmpty else {
      return Self.errorResponse(
        req, code: .invalidRequest, message: "INVALID_REQUEST: bins required")
    }
  ...
  private func handleSystemExecApprovalsGet(_ req: BridgeInvokeRequest) async throws
    -> BridgeInvokeResponse
  {
  ...
  private func handleSystemExecApprovalsSet(_ req: BridgeInvokeRequest) async throws
    -> BridgeInvokeResponse
  {
  ...
  private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
    ...
    if title.isEmpty, body.isEmpty {
      return Self.errorResponse(
        req, code: .invalidRequest, message: "INVALID_REQUEST: empty notification")
    }

    let priority = params.priority.flatMap { NotificationPriority(rawValue: $0.rawValue) }
    let delivery =
      params.delivery.flatMap { NotificationDelivery(rawValue: $0.rawValue) } ?? .system
...
extension MacNodeRuntime {
  ...
  private func recordAllowlistMatches(
    ...
    for (idx, match) in allowlistMatches.enumerated() {
      ...
      let resolvedPath =
        idx < allowlistResolutions.count ? allowlistResolutions[idx].resolvedPath : nil
  ...
  static func projectedOuterFrameBytes(
    ...
    let frame = InvokeResultFrame(
      params: InvokeResultFrame.Params(
        id: requestId,
        nodeId: nodeId ?? "",
        ok: true,
  ...
  private static func screenSnapshotPayloadTooLarge(_ req: BridgeInvokeRequest)
    -> BridgeInvokeResponse
  {
    self.errorResponse(
      req,
