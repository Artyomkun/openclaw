...
extension NodeServiceManager {
  ...
  private static func runServiceCommandResult(
    ...
    let response = await ShellExecutor.runDetailed(
      command: command, cwd: nil, env: env, timeout: timeout)
    let parsed =
      self.parseServiceJson(from: response.stdout) ?? self.parseServiceJson(from: response.stderr)
    let ok = parsed?.ok
    let message = parsed?.error ?? parsed?.message
    let payload =
      parsed?.text.data(using: .utf8)
    ...
    let fullMessage =
      detail.map { "Node service command failed (\(exit)): \($0)" }
  ...
  private static func errorMessage(from result: CommandResult, treatNotLoadedAsError: Bool)
    -> String?
  {
    if !result.success {
