...
final class OnboardingWizardModel {
    ...
    do {
      ...
      if await GatewayProcessManager.shared.waitForGatewayReady(timeout: 12) == false {
        throw NSError(
          ...
          code: 1,
          userInfo: [
            NSLocalizedDescriptionKey: "Gateway did not become ready. Check that it is running."
          ])
      }
  ...
  private func restartIfSessionLost(error: Error) -> Bool {
    ...
    guard message.contains("wizard not found") || message.contains("wizard not running") else {
      return false
    }
...
struct OnboardingWizardStepView: View {
  ...
  init(step: WizardStep, isSubmitting: Bool, onSubmit: @escaping (AnyCodable?) -> Void) {
    ...
    let initialIndex =
      options.firstIndex(where: { anyCodableEqual($0.option.value, step.initialvalue) }) ?? 0
