...
enum OpenClawConfigFile {
    ...
    -> Bool
  {
    self.withFileLock {
      ...
      var output =
        if preserveExistingKeys, let previousRoot {
      ...
      do {
        ...
        if !blocking.isEmpty {
          ...
          self.logger.warning(
            "config write rejected (\(blocking.joined(separator: ", "))) at \(url.path)")
          self.appendConfigWriteAudit([
            ...
            "nextDev": NSNull(),
            "previousIno": self.fileSystemNumber(previousAttributes?[.systemFileNumber])
              ?? NSNull(),
            "nextIno": NSNull(),
            "previousMode": self.posixMode(previousAttributes?[.posixPermissions]) ?? NSNull(),
            "nextMode": NSNull(),
            "previousNlink": self.fileAttributeInt(previousAttributes?[.referenceCount])
              ?? NSNull(),
            "nextNlink": NSNull(),
            "previousUid": self.fileAttributeInt(previousAttributes?[.ownerAccountID]) ?? NSNull(),
            "nextUid": NSNull(),
            "previousGid": self.fileAttributeInt(previousAttributes?[.groupOwnerAccountID])
              ?? NSNull(),
            "nextGid": NSNull(),
        ...
        if !suspicious.isEmpty {
          self.logger.warning(
            "config write anomaly (\(suspicious.joined(separator: ", "))) at \(url.path)")
        }
        self.appendConfigWriteAudit([
          "result": "success",
          "configPath": url.path,
  ...
  private static func configDictionariesEqual(_ left: [String: Any]?, _ right: [String: Any])
    -> Bool
  {
  ...
  private static func mergeExistingConfig(
    ...
    overridingWith next: [String: Any]
  ) -> [String: Any] {
    var merged = existing
  ...
  private static func observeSuspiciousReasons(
    ...
    if let root, (lastKnownGood["gatewayMode"] as? String) != nil,
      self.isUpdateChannelOnlyRoot(root)
    {
  ...
  private static func persistClobberedSnapshot(data: Data, configURL: URL, observedAt: String)
    -> String?
  {
    let url = configURL.deletingLastPathComponent()
      .appendingPathComponent(
        "\(configURL.lastPathComponent).clobbered.\(self.configTimestampToken(observedAt))")
  ...
  private static func persistRejectedConfigWrite(data: Data, configURL: URL) -> String? {
    ...
    let url = configURL.deletingLastPathComponent()
      .appendingPathComponent(
        "\(configURL.lastPathComponent).rejected.\(self.configTimestampToken(timestamp))")
    let fileManager = FileManager()
    let privatePermissions: NSNumber = 0o600
    if fileManager.fileExists(atPath: url.path) {
      try? fileManager.setAttributes(
        [.posixPermissions: privatePermissions], ofItemAtPath: url.path)
      return url.path
    }
    guard
  ...
  private static func observeConfigRead(
    data: Data, root: [String: Any]?, configURL: URL, valid: Bool
  ) {
    let observedAt = ISO8601DateFormatter().string(from: Date())
    let current = self.configFingerprint(
      data: data, root: root, configURL: configURL, observedAt: observedAt)
    ...
    if suspicious.isEmpty {
      ...
      if !self.sameFingerprint(lastKnownGood, current)
        || entry["lastObservedSuspiciousSignature"] != nil
      {
        state = self.setConfigHealthEntry(
          state: state, configPath: configURL.path, entry: nextEntry)
    ...
    let backup = self.readConfigFingerprint(
      at: configURL.deletingLastPathComponent().appendingPathComponent(
        "\(configURL.lastPathComponent).bak"))
    let clobberedPath = self.persistClobberedSnapshot(
      data: data,
      configURL: configURL,
      observedAt: observedAt)
    self.logger.warning(
      "config observe anomaly (\(suspicious.joined(separator: ", "))) at \(configURL.path)")
