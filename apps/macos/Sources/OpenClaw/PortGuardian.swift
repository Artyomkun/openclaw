...
actor PortGuardian {
    ...
    for port in ports {
      ...
      for listener in listeners {
        ...
        if killed {
          ...
          self.logger.error(
            "failed to terminate pid \(listener.pid) on port \(port, privacy: .public)")
        }
      }
    }
  ...
  struct PortReport: Identifiable {
    ...
    var offenders: [ReportListener] {
      if case .interference(_, let offenders) = self.status { return offenders }
      return []
    }

    var summary: String {
      switch self.status {
      case .ok(let text): text
      case .missing(let text): text
      case .interference(let text, _): text
      }
    }
  }
  ...
  private func kill(_ pid: Int32) async -> Bool {
    let term = await ShellExecutor.run(
      command: ["kill", "-TERM", "\(pid)"], cwd: nil, env: nil, timeout: 2)
    if term.ok { return true }
    let sigkill = await ShellExecutor.run(
      command: ["kill", "-KILL", "\(pid)"], cwd: nil, env: nil, timeout: 2)
    return sigkill.ok
  }

  private static func isExpected(_ listener: Listener, port: Int, mode: AppState.ConnectionMode)
    -> Bool
  {
    ...
    case .remote:
      ...
      return false
    case .local:
      // Preserve both the hidden alias and the current service process title.
  ...
  private func probeGatewayHealthIfNeeded(
    ...
    guard mode == .remote, port == GatewayEnvironment.gatewayPort(), !listeners.isEmpty else {
      return nil
    }
  ...
  extension PortGuardian {
    ...
    static func _testBuildReport(
      port: Int,
      mode: AppState.ConnectionMode,
      listeners: [(pid: Int32, command: String, fullCommand: String, user: String?)]
    ) -> PortReport {
      let mapped = listeners.map {
        Listener(
          pid: $0.pid,
          command: $0.command,
