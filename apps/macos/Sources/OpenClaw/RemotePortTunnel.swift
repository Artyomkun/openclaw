...
final class RemotePortTunnel: @unchecked Sendable {
  ...
  static func create(
    ...
    guard settings.mode == .remote, let parsed = CommandResolver.parseSSHTarget(settings.target)
    else {
      throw NSError(
  ...
  private static func waitForListener(
    ...
    repeat {
      if !process.isRunning {
        ...
        let msg =
          stderr.isEmpty ? "ssh tunnel exited before listening" : "ssh tunnel failed: \(stderr)"
        throw NSError(
          domain: "RemotePortTunnel", code: 4, userInfo: [NSLocalizedDescriptionKey: msg])
      }
    ...
    let msg =
      stderr.isEmpty
      ? "ssh tunnel did not open local port \(localPort)" : "ssh tunnel failed: \(stderr)"
