...
enum SessionMenuPreviewLoader {
  ...
  private static func fetchSnapshot(sessionKey: String, maxItems: Int) async throws
    -> SessionMenuPreviewSnapshot
  {
    do {
      let payload = try await self.requestPreview(keys: [sessionKey], maxItems: maxItems)
      if let entry = payload.previews.first(where: { $0.key == sessionKey })
        ?? payload.previews.first
      {
  ...
  private static func fetchHistorySnapshot(
    sessionKey: String,
    maxItems: Int
  ) async throws -> SessionMenuPreviewSnapshot {
    let timeoutMs = Int(self.previewTimeoutSeconds * 1000)
  ...
  private static func isToolCall(_ message: OpenClawChatMessage) -> Bool {
    ...
    return message.content.contains {
      $0.name?.nonEmpty != nil || $0.type?.lowercased() == "toolcall"
    }
  }
