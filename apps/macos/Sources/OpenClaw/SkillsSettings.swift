...
private struct SkillRow: View {
  ...
  private var shouldShowMissingSummary: Bool {
    self.shouldShowMissingBins || !self.missingEnv.isEmpty || !self.missingConfig.isEmpty
  }
...
final class SkillsSettingsModel {
  ...
  fileprivate func install(skill: SkillStatus, option: SkillInstallOption, target: InstallTarget)
    async
  {
      ...
      do {
        if isPrimary {
          _ = try await GatewayConnection.shared.skillsUpdate(
            ...
            apiKey: value)
          self.statusMessage =
            "Saved API key — stored in openclaw.json (skills.entries.\(skillKey))"
        } else {
          _ = try await GatewayConnection.shared.skillsUpdate(
            skillKey: skillKey,
            env: [envKey: value])
          self.statusMessage =
            "Saved \(envKey) — stored in openclaw.json (skills.entries.\(skillKey).env)"
        }
      } catch {
        self.statusMessage = error.localizedDescription
      }
    ...
    static func exerciseForTesting() {
      let skill = SkillStatus(
        ...
        requirements: SkillRequirements(
          bins: ["python3"], env: ["API_KEY"], config: ["skills.test"]),
        ...
        install: [
          SkillInstallOption(
            id: "brew", kind: "brew", label: "brew install python", bins: ["python3"])
        ])
