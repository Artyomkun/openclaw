...
struct SystemRunSettingsView: View {
  ...
  private var summaryPanel: some View {
    ...
    .background(
      .quaternary.opacity(0.34), in: RoundedRectangle(cornerRadius: 14, style: .continuous)
    )
    .overlay {
  ...
  private var defaultsAllowlistEmptyState: some View {
    ...
    .frame(maxWidth: .infinity, alignment: .leading)
    .background(
      .quaternary.opacity(0.26), in: RoundedRectangle(cornerRadius: 12, style: .continuous))
  }
...
final class ExecApprovalsSettingsModel {
  ...
  func addEntry(_ pattern: String) -> ExecAllowlistPatternValidationReason? {
    ...
    case .valid(let normalizedPattern):
      ...
      let rejected = ExecApprovalsStore.updateAllowlist(
        agentId: self.selectedAgentId, allowlist: self.entries)
  ...
  private func syncQuickMode() {
    if self.isDefaultsScope {
      AppStateStore.shared.execApprovalMode = ExecApprovalQuickMode.from(
        security: self.security, ask: self.ask)
      return
    }
