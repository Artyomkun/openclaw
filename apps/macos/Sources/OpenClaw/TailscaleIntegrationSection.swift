...
private typealias GatewayTailscaleSettingsSaver =
  @MainActor @Sendable (
...
struct TailscaleIntegrationSection: View {
  ...
  private static func applySettingsIfChanged(
    ...
    connectionMode: AppState.ConnectionMode,
    isPaused: Bool,
    saveSettings: GatewayTailscaleSettingsSaver
  ...
  private static func messages(
    ...
    let statusMessage =
      if connectionMode == .local, !isPaused {
  ...
  extension TailscaleIntegrationSection {
    ...
    static func simulateHydrationApplyForTesting(
      ...
      connectionMode: AppState.ConnectionMode,
      isPaused: Bool,
      saveRoot: @MainActor @Sendable @escaping ([String: Any]) -> Void
    ) async {
