...
enum TalkModeGatewayConfigParser {
  static func parse(
    ...
    let rawSeam =
      ui?["seamColor"]?.stringValue?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
    ...
    let model = activeConfig?["modelId"]?.stringValue?.trimmingCharacters(
  ...
  static func fallback(
    ...
    let resolvedVoice =
      (envVoice?.isEmpty == false ? envVoice : nil) ?? (sagVoice?.isEmpty == false ? sagVoice : nil)
