...
actor TalkModeRuntime {
  ...
  private func sendAndSpeak(_ transcript: String) async {
    ...
    self.logger.info(
      "talk send start runId=\(runId, privacy: .public) "
        + "session=\(sessionKey, privacy: .public) " + "chars=\(prompt.count, privacy: .public)")
    ...
    return await withTaskGroup(of: String?.self) { group in
      group.addTask { [runId, sessionKey] in
        ...
        for await push in stream {
          ...
          guard case .event(let evt) = push else { continue }
  ...
  private func latestAssistantText(sessionKey: String, since: Double? = nil) async -> String? {
    do {
      ...
      self.logger.error(
        "talk history fetch failed: \(error.localizedDescription, privacy: .public)")
      return nil
    }
  }
  ...
  private func playAssistant(text: String) async {
    ...
    {
    case .elevenLabsThenSystemVoice(let apiKey, let voiceId):
      do {
        ...
        do {
          ...
          self.ttsLogger.error(
            "talk system voice failed: \(error.localizedDescription, privacy: .public)")
        }
      }
    case .gatewayTalkSpeakThenSystemVoice:
      do {
  ...
  private func preparePlaybackInput(text: String) async -> TalkPlaybackInput? {
    ...
    let preferredVoice =
      resolvedVoice ?? self.currentVoiceId ?? self.defaultVoiceId
    ...
    let voiceId: String? =
      if provider == Self.defaultTalkProvider, let apiKey, !apiKey.isEmpty {
        await self.resolveVoiceId(preferred: preferredVoice, apiKey: apiKey)
      } else if provider == Self.mlxTalkProvider || provider == Self.systemTalkProvider {
        nil
      } else {
        preferredVoice?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false
          ? preferredVoice : nil
      }
  ...
  private func playElevenLabs(
    ...
    let desiredOutputFormat =
      input.directive?.outputFormat ?? self.defaultOutputFormat ?? "pcm_44100"
    ...
    if !result.finished, result.interruptedAt == nil {
      throw NSError(
        domain: "StreamingAudioPlayer", code: 1,
        userInfo: [
  ...
  private func resolveVoiceId(preferred: String?, apiKey: String) async -> String? {
    ...
    do {
      ...
      self.ttsLogger
        .info(
          "talk default voice selected \(name, privacy: .public) (\(first.voiceId, privacy: .public))"
        )
    ...
    self.logger
      .info(
        "talk config provider=\(cfg.activeProvider, privacy: .public) "
          ...
          + "modelId=\(modelLabel, privacy: .public) " + "apiKey=\(hasApiKey, privacy: .public) "
