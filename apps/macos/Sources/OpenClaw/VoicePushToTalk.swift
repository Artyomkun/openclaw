...
final class VoicePushToTalkHotkey: @unchecked Sendable {
  ...
  private func startMonitoring() {
    ...
    self.globalMonitor = NSEvent.addGlobalMonitorForEvents(matching: .flagsChanged) {
      [weak self] event in
    ...
    self.localMonitor = NSEvent.addLocalMonitorForEvents(matching: .flagsChanged) {
      [weak self] event in
      let keyCode = event.keyCode
      let flags = event.modifierFlags
      self?.handleFlagsChanged(keyCode: keyCode, modifierFlags: flags)
...
actor VoicePushToTalk {
  ...
  func begin() async {
    ...
    self.adoptedPrefix =
      snapshot.visible ? snapshot.text.trimmingCharacters(in: .whitespacesAndNewlines) : ""
    ...
    let adoptedAttributed: NSAttributedString? =
      adoptedPrefix.isEmpty
      ? nil
  ...
  private func finalize(transcriptOverride: String?, reason: String, sessionID: UUID?) async {
    ...
    await MainActor.run {
      logger.info(
        "ptt finalize reason=\(reason, privacy: .public) len=\(finalText.count, privacy: .public)")
      if let token {
  ...
  static func _testAttributedColors(isFinal: Bool) -> (NSColor, NSColor) {
    let sample = VoiceOverlayTextFormatting.makeAttributed(
      committed: "a", volatile: "b", isFinal: isFinal)
    let committedColor =
      sample.attribute(.foregroundColor, at: 0, effectiveRange: nil) as? NSColor ?? .clear
    let volatileColor =
      sample.attribute(.foregroundColor, at: 1, effectiveRange: nil) as? NSColor ?? .clear
