...
enum VoiceWakeForwarder {
  ...
  enum VoiceWakeForwardError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
      switch self {
      case .rpcFailed(let message): message
      }
    }
  }
  ...
  static func selectedSessionOptions(voiceWakeTrigger: String? = nil) async -> ForwardOptions {
    ...
    let sessionKey: String =
      if let activeSessionKey = activeSessionKey?.trimmingCharacters(
  ...
  static func forwardToSelectedSession(
    transcript: String,
    voiceWakeTrigger: String? = nil
  ) async -> Result<Void, VoiceWakeForwardError> {
    ...
    do {
      ...
      return response.sessions.first {
        $0.key.trimmingCharacters(in: .whitespacesAndNewlines)
          .caseInsensitiveCompare(sessionKey.trimmingCharacters(in: .whitespacesAndNewlines))
          == .orderedSame
      }
    } catch {
      self.logger.debug(
  ...
  private static func parseSessionKeyRoute(_ sessionKey: String) -> (channel: String, to: String?)?
  {
    ...
    let body: [String] =
      if rawParts.count >= 3, rawParts[0].caseInsensitiveCompare("agent") == .orderedSame {
