...
extension VoiceWakeOverlayController {
  ...
  private func ensureWindow() {
    ...
    let panel = OverlayPanelFactory.makePanel(
      contentRect: NSRect(
        x: 0, y: 0, width: self.width + borderPad * 2, height: 60 + borderPad * 2),
  ...
  func targetFrame() -> NSRect {
    ...
    let size = NSSize(
      width: self.width + self.closeOverflow * 2, height: height + self.closeOverflow * 2)
  ...
  func updateWindowFrame(animate: Bool = false) {
    OverlayPanelFactory.applyFrame(
      window: self.window, target: self.targetFrame(), animate: animate)
  }

  func measuredHeight() -> CGFloat {
    let attributed =
      self.model.attributed.length > 0
      ? self.model.attributed
      : self
    ...
    let container = NSTextContainer(
      containerSize: CGSize(width: containerWidth, height: .greatestFiniteMagnitude))
  ...
  func dismissTargetFrame(for frame: NSRect, reason: DismissReason, outcome: SendOutcome) -> NSRect?
  {
    ...
    case (.empty, _):
      ...
      return NSRect(
        x: frame.origin.x + dx, y: frame.origin.y + dy, width: newSize.width, height: newSize.height
      )
    case (.explicit, .sent):
