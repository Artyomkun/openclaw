...
actor VoiceWakeRuntime {
  ...
  private func start(with config: RuntimeConfig) async {
    ...
    do {
      ...
      self.recognitionTask = recognizer.recognitionTask(with: request) {
        ...
        let segments =
          result.flatMap { result in
            transcript
              .map {
                WakeWordSpeechSegments.from(transcription: result.bestTranscription, transcript: $0)
              }
          } ?? []
        let isFinal = result?.isFinal ?? false
        Task {
          await self.noteRecognitionCallback(transcript: transcript, isFinal: isFinal, error: error)
        }
      ...
      DiagnosticsFileLog.shared.log(
        category: "voicewake.runtime", event: "started",
        fields: [
      ...
      self.logger.error(
        "voicewake runtime failed to start: \(error.localizedDescription, privacy: .public)")
      self.stop()
    }
  }
  ...
  private func configureSession(localeID: String?) {
    let locale =
      localeID.flatMap { Locale(identifier: $0) } ?? Locale(identifier: Locale.current.identifier)
  ...
  private func handleRecognition(_ update: RecognitionUpdate, config: RuntimeConfig) async {
    ...
    if let error = update.error {
      self.logger.debug(
        "voicewake recognition error: \(error.localizedDescription, privacy: .public)")
    }
    ...
    if let match {
      ...
      if usedFallback {
        self.logger.info(
          "voicewake runtime detected (text-only fallback) len=\(match.command.count)")
      } else {
  ...
  private func beginCapture(
    ...
    if config.triggersTalkMode {
      ...
      if config.triggerChime != .none {
        await MainActor.run {
          VoiceWakeChimePlayer.play(config.triggerChime, reason: "voicewake.trigger")
        }
      }
  ...
  private func monitorCapture(config: RuntimeConfig) async {
    ...
    while self.isCapturing {
      ...
      let silenceThreshold =
        self.heardBeyondTrigger ? self.silenceWindow : self.triggerOnlySilenceWindow
  ...
  private func finalizeCapture(config: RuntimeConfig) async {
    ...
    DiagnosticsFileLog.shared.log(
      category: "voicewake.runtime", event: "finalizeCapture",
      fields: [
