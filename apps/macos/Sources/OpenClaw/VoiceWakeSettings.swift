...
struct VoiceWakeSettings: View {
  ...
  private var voiceSummaryPanel: some View {
    ...
    let statusColor: Color =
      !voiceWakeSupported ? .orange : enabled || pushToTalk ? .green : .secondary
    ...
    Task { @MainActor in
      do {
        ...
        self.testTimeoutTask = Task { @MainActor in
          ...
          if self.isTesting {
            self.tester.stop()
            if case .hearing(let text) = self.testState,
  ...
  private var additionalLanguageRows: some View {
    VStack(alignment: .leading, spacing: 0) {
      ForEach(Array(self.state.voiceWakeAdditionalLocaleIDs.enumerated()), id: \.offset) {
        idx, localeID in
  ...
  private func additionalLocaleBinding(index: Int, fallback: String) -> Binding<String> {
    Binding(
      get: {
        guard self.state.voiceWakeAdditionalLocaleIDs.indices.contains(index) else {
          return fallback
        }
  ...
  private func addAdditionalLocale() {
    ...
    let next =
      self.availableLocales.first { !selected.contains($0.identifier) }
      ?? self.availableLocales.first
    if let next {
