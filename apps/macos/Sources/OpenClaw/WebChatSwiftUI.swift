...
struct MacGatewayChatTransport: OpenClawChatTransport {
    ...
    AsyncStream { continuation in
      let task = Task {
        do {
          ...
          webChatSwiftLogger.error(
            "gateway refresh failed \(error.localizedDescription, privacy: .public)")
        }
  ...
  private static func mapModelChoice(_ model: OpenClawProtocol.ModelChoice)
...
final class WebChatSwiftUIWindowController {
  ...
  convenience init(sessionKey: String, presentation: WebChatPresentation) {
    self.init(
      sessionKey: sessionKey, presentation: presentation, transport: MacGatewayChatTransport())
  }
  ...
  private static func persistedThinkingLevel() -> String? {
    let stored = UserDefaults.standard.string(forKey: webChatThinkingLevelDefaultsKey)?
      ...
      .lowercased()
    guard let stored,
      ["off", "minimal", "low", "medium", "high", "xhigh", "adaptive"].contains(stored)
    else {
      return nil
    }
    return stored
  ...
  private static func makeContentController(
    for presentation: WebChatPresentation,
    hosting: NSHostingController<OpenClawChatView>
  ) -> NSViewController {
