...
final class WorkActivityStore {
  ...
  func resolveIconState(override selection: IconOverrideSelection) {
    ...
    default:
      ...
      switch base {
      case .workingMain(let kind),
        .workingOther(let kind):
        self.iconState = .overridden(kind)
      case .overridden(let kind):
        self.iconState = .overridden(kind)
      case .idle:
        self.iconState = .idle
      }
  ...
  private func lastUpdate(for sessionKey: String) -> Date {
    max(
      self.jobs[sessionKey]?.lastUpdate ?? .distantPast,
      self.tools[sessionKey]?.lastUpdate ?? .distantPast)
  }
  ...
  private static func wrapToolArgs(_ args: [String: OpenClawProtocol.AnyCodable]?) -> OpenClawKit
    .AnyCodable?
  {
