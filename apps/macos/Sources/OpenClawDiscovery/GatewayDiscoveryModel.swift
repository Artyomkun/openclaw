...
public final class GatewayDiscoveryModel {
  ...
  private func mapWideAreaBeacons(_ beacons: [WideAreaGatewayBeacon], domain: String)
    -> [DiscoveredGateway]
  {
  ...
  private func mapTailscaleServeBeacons(
    _ beacons: [TailscaleServeGatewayBeacon]
  ) -> [DiscoveredGateway] {
    beacons.map { beacon in
  ...
  private func updateGateways(for domain: String) {
    ...
    self.gatewaysByDomain[domain] = results.compactMap { result -> DiscoveredGateway? in
      guard case .service(let name, let type, let resultDomain, _) = result.endpoint else {
        return nil
      }
  ...
  static func shouldContinueTailscaleServeDiscovery(
    ...
    tailscaleServeGateways: [DiscoveredGateway]
  ) -> Bool {
  ...
  private nonisolated static func prettifyServiceName(_ decodedName: String) -> String {
    ...
    var cleaned = normalized.replacingOccurrences(
      of: #"\s*-?gateway$"#, with: "", options: .regularExpression)
    cleaned =
      cleaned
  ...
  private nonisolated static func normalizeHostToken(_ raw: String?) -> String? {
    ...
    let strippedTrailingDot =
      lower.hasSuffix(".")
      ? String(lower.dropLast())
      : lower
    let withoutLocal =
      strippedTrailingDot.hasSuffix(".local")
