...
public enum Request: Sendable {
  ...
  case screenRecord(
    screenIndex: Int?, durationMs: Int?, fps: Double?, includeAudio: Bool, outPath: String?)
}
...
extension Request: Codable {
  ...
  public func encode(to encoder: Encoder) throws {
    ...
    switch self {
    case .notify(let title, let body, let sound, let priority, let delivery):
    ...
    case .ensurePermissions(let caps, let interactive):
    ...
    case .runShell(let command, let cwd, let env, let timeoutSec, let needsSR):
    ...
    case .agent(let message, let thinking, let session, let deliver, let to):
    ...
    case .nodeDescribe(let nodeId):
      try container.encode(Kind.nodeDescribe, forKey: .type)
      try container.encode(nodeId, forKey: .nodeId)

    case .nodeInvoke(let nodeId, let command, let paramsJSON):
    ...
    case .cameraSnap(let facing, let maxWidth, let quality, let outPath):
    ...
    case .cameraClip(let facing, let durationMs, let includeAudio, let outPath):
    ...
    case .screenRecord(let screenIndex, let durationMs, let fps, let includeAudio, let outPath):
  ...
  public init(from decoder: Decoder) throws {
    ...
    case .runShell:
      ...
      self = .runShell(
        command: command, cwd: cwd, env: env, timeoutSec: timeout, needsScreenRecording: needsSR)
    ...
    case .agent:
      ...
      self = .agent(
        message: message, thinking: thinking, session: session, deliver: deliver, to: to)
    ...
    case .cameraClip:
      ...
      let outPath = try container.decodeIfPresent(String.self, forKey: .outPath)
      self = .cameraClip(
        facing: facing, durationMs: durationMs, includeAudio: includeAudio, outPath: outPath)
