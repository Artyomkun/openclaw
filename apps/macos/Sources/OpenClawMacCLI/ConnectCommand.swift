...
struct ConnectOptions {
  ...
  static func parse(_ args: [String]) -> ConnectOptions {
    ...
    let valueHandlers: [String: (inout ConnectOptions, String) -> Void] = [
      ...
      "--role": { $0.role = $1 },
      "--scopes": { opts, raw in
        opts.scopes = raw.split(separator: ",").map {
    ...
    while i < args.count {
      ...
      if let handler = valueHandlers[arg],
        let value = CLIArgParsingSupport.nextValue(args, index: &i)
      {
        handler(&opts, value)
        i += 1
        continue
...
private func resolveGatewayEndpoint(opts: ConnectOptions, config: GatewayConfig) throws
  -> GatewayEndpoint
{
...
) throws -> GatewayEndpoint {
  guard let url = URL(string: raw) else {
    throw NSError(
      domain: "Gateway", code: 1, userInfo: [NSLocalizedDescriptionKey: "invalid url: \(raw)"])
  }
  return GatewayEndpoint(
    url: url,
