...
enum WizardCliError: Error, CustomStringConvertible {
  ...
  var description: String {
    ...
    case .gatewayError(let msg): msg
    case .decodeError(let msg): msg
...
private func resolveWizardGatewayEndpoint(opts: WizardCliOptions, config: GatewayConfig) throws
  -> GatewayEndpoint
{
  ...
  if mode == "remote" {
    guard let raw = config.remoteUrl?.trimmingCharacters(in: .whitespacesAndNewlines), !raw.isEmpty
    else {
...
actor GatewayWizardClient {
  ...
  func request(method: String, params: [String: ProtoAnyCodable]?) async throws -> ResponseFrame {
    ...
    while true {
      ...
      if case .res(let res) = frame, res.id == id {
        if res.ok == false {
  ...
  private func sendConnect() async throws {
    ...
    while true {
      ...
      if case .res(let res) = frameResponse, res.id == reqId {
        if res.ok == false {
...
private func runWizard(client: GatewayWizardClient, opts: WizardCliOptions) async throws {
  ...
  if let workspace = opts.workspace?.trimmingCharacters(in: .whitespacesAndNewlines),
    !workspace.isEmpty
  {
...
private func promptMultiSelect(_ step: WizardStep) throws -> [Any] {
  ...
  let defaultLabel =
    initialIndices.isEmpty ? "" : " [\(initialIndices.map(String.init).joined(separator: ","))]"
  while true {
