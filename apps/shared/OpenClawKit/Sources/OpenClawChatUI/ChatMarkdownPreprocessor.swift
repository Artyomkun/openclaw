...
enum ChatMarkdownPreprocessor {
  ...
  private static func stripInboundContextBlocks(_ raw: String) -> String {
    guard
      self.inboundContextHeaders.contains(where: raw.contains)
        || raw.contains(self.untrustedContextHeader)
    else {
      return raw
    }
    ...
    for index in lines.indices {
      ...
      if !inMetaBlock,
        self.inboundContextHeaders.contains(
  ...
  private static func shouldStripTrailingUntrustedContext(lines: [String], index: Int) -> Bool {
    guard
      lines[index].trimmingCharacters(in: .whitespacesAndNewlines) == self.untrustedContextHeader
    else {
      return false
    }
