...
struct ChatAgentAvatar: View {
  ...
  private var displayText: String {
    ...
    if let name = self.name?.trimmingCharacters(in: .whitespacesAndNewlines), !name.isEmpty {
      let words = name.split(whereSeparator: { $0.isWhitespace || $0 == "-" || $0 == "_" }).prefix(
        2)
...
private struct ChatMessageBody: View {
  ...
  var body: some View {
    ...
    .shadow(
      color: self.bubbleShadowColor, radius: self.bubbleShadowRadius, y: self.bubbleShadowYOffset
    )
...
private struct ToolResultCard: View {
  ...
  private var displayText: String {
    guard !self.expanded, self.lines.count > Self.previewLineLimit else {
...
private struct ChatAssistantTextBody: View {
  ...
  var body: some View {
    let segments = AssistantTextParser.segments(
      from: self.text, includeThinking: self.includesThinking)
    VStack(alignment: .leading, spacing: 10) {
