...
public struct OpenClawChatView: View {
  ...
  private var activeErrorText: String? {
    guard
      let text = self.viewModel.errorText?
  ...
  private var showsCleanLoadingPlaceholder: Bool {
    self.composerChrome == .clean && self.viewModel.isLoading
      && self.visibleEmptyAssistantIntro == nil && self.activeErrorText == nil
      && !self.hasVisibleMessageListContent
  }
  ...
  private var showsEmptyState: Bool {
    self.viewModel.messages.isEmpty
      ...
      } ?? false) && self.viewModel.pendingRunCount == 0 && self.viewModel.pendingToolCalls.isEmpty
  }
  ...
  private func errorPresentation(for error: String) -> (
  ...
  private func shouldDisplayMessage(_ message: OpenClawChatMessage) -> Bool {
    ...
    if !primaryText.isEmpty {
      ...
      if AssistantTextParser.hasVisibleContent(
        in: primaryText, includeThinking: self.showsAssistantTrace)
      {
        return true
      }
    }
