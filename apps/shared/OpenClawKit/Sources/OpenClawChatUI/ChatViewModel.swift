...
public final class OpenClawChatViewModel {
  ...
  public var sessionChoices: [OpenClawChatSessionEntry] {
    ...
    for entry in sorted {
      ...
      guard entry.key == self.sessionKey || !Self.isHiddenInternalSession(entry.key) else {
        continue
      }
  ...
  private func applyHistoryPayload(
    ...
    self.messages =
      if preservingOptimisticLocalMessages {
    ...
    let canInvalidateOlderHistory =
      if let latestUserTurn = request.latestUserTurn {
  ...
  private func adoptPendingLocalUserEcho(incoming: OpenClawChatMessage) -> Bool {
    ...
    guard
      let matchIndex = messages.lastIndex(where: { existing in
  ...
  private static func reconcileRunRefreshMessages(
    ...
    let trailingLocalCandidates =
      lastCanonicalPreviousIndex.map { index in
    ...
    for message in optimisticUserMessages {
      ...
      let insertIndex =
        reconciled.firstIndex { existing in
  ...
  private func performSend() async {
    ...
    do {
      ...
      if response.runId != runId {
        let pendingUserMessageID = self.pendingLocalUserEchoMessageIDsByRunID.removeValue(
          forKey: runId)
      ...
      chatUILogger.error(
        "chat transport send failed \(error.localizedDescription, privacy: .public)")
    }
  }
  ...
  private func performSelectModel(_ selectionID: String) async {
    ...
    do {
      ...
      self.errorText = error.localizedDescription
      chatUILogger.error(
        "sessions.patch(model) failed \(error.localizedDescription, privacy: .public)")
    }
  }
  ...
  ) -> [OpenClawChatThinkingLevelOption] {
    if let levels = Self.normalizedThinkingLevelOptions(currentSession?.thinkingLevels),
      !levels.isEmpty
    {
      return levels
    }
  ...
  private func normalizedModelSelectionID(_ modelID: String?, provider: String? = nil) -> String? {
    ...
    if let provider = Self.normalizedProvider(provider) {
      let providerQualified = Self.providerQualifiedModelSelectionID(
        modelID: trimmed, provider: provider)
  ...
  private func generatedNewSessionKey() -> String {
    ...
    guard
      let agentID = Self.agentID(fromSessionKey: sessionKey) ?? Self.agentID(
        fromSessionKey: resolvedMainSessionKey)
  ...
  private func modelLabel(for modelID: String) -> String {
    self.modelChoices.first(where: { $0.selectionID == modelID || $0.modelID == modelID })?
      .displayLabel ?? modelID
  }

  private func applySuccessfulModelSelection(
    _ selectionID: String, sessionKey: String, syncSelection: Bool
  ) {
  ...
  private func handleTransportEvent(_ evt: OpenClawChatTransportEvent) {
    ...
    case .sessionMessage(let message):
  ...
  private func handleSessionMessageEvent(_ payload: OpenClawSessionMessageEventPayload) {
    if let sessionKey = payload.sessionKey,
      !self.matchesCurrentSessionKey(
        incoming: sessionKey, agentId: payload.agentId, current: self.sessionKey)
    {
      return
    }
    ...
    let reconciled = Self.reconcileMessageIDs(
      previous: self.messages, incoming: self.messages + [sanitized])
  ...
  private func appendFinalChatMessageIfPresent(_ chat: OpenClawChatEventPayload) {
    ...
    let reconciled = Self.reconcileMessageIDs(
      previous: self.messages, incoming: self.messages + [message])
    self.messages = Self.dedupeMessages(reconciled)
  }
  ...
  private static func messageWithTimestampIfNeeded(_ message: OpenClawChatMessage)
    -> OpenClawChatMessage
  {
  ...
  private func handleAgentLifecycleEvent(_ evt: OpenClawAgentEventPayload, isPendingRun: Bool) {
    ...
    let isFailure =
      phase == "error" || phase == "failed" || phase == "aborted" || status == "error"
      || status == "failed" || status == "aborted"
    let isSuccessfulStatus =
      status == "ok" || status == "success" || status == "succeeded" || status == "complete"
      || status == "completed"
  ...
  private static func agentLifecycleErrorMessage(_ evt: OpenClawAgentEventPayload, aborted: Bool)
    -> String
  {
    if aborted {
      return "Run aborted"
  ...
  private func armRunCompletionRefresh(
    ...
    Task { [weak self, transport] in
      let observedCompletion = await transport.waitForRunCompletion(
        runId: runId, timeoutMs: timeoutMs)
  ...
  @discardableResult
  private func clearPendingRunIfAssistantMessagePresent(runId: String, after timestamp: Double)
    -> Bool
  {
  ...
  private static func hasUnansweredLatestUser(in messages: [OpenClawChatMessage]) -> Bool {
    self.latestUserTurn(in: messages) != nil
      && !self.hasAssistantMessageAfterLatestUser(in: messages)
  }
