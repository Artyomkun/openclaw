...
public struct GatewayConnectDeepLink: Codable, Sendable, Equatable {
  ...
  public init(
    host: String, port: Int, tls: Bool, bootstrapToken: String?, token: String?, password: String?
  ) {
    self.host = host
    self.port = port
    self.tls = tls
...
public enum DeepLinkParser {
  public static func parse(_ url: URL) -> DeepLinkRoute? {
    ...
    case "agent":
      ...
      let timeoutSeconds = query["timeoutSeconds"].flatMap { Int($0) }.flatMap {
        $0 >= 0 ? $0 : nil
      }
      return .agent(
        .init(
