...
private enum GatewayConnectErrorCodes {
  ...
  static let authPasswordNotConfigured = GatewayConnectAuthDetailCode.authPasswordNotConfigured
    .rawValue
  ...
  static let controlUiDeviceIdentityRequired = GatewayConnectAuthDetailCode
    .controlUiDeviceIdentityRequired.rawValue
...
public actor GatewayChannelActor {
  ...
  private func watchdogLoop() async {
    ...
    while self.shouldReconnect {
      ...
      do {
        ...
        self.logger.error(
          "gateway watchdog reconnect failed \(wrapped.localizedDescription, privacy: .public)")
      }
    }
  }
  ...
  public func connect() async throws {
    ...
    do {
      ...
      self.logger.error(
        "gateway ws connect failed \(wrapped.localizedDescription, privacy: .public)")
      throw wrapped
    }
    self.listen()
    self.connected = true
  ...
  private func keepaliveLoop() async {
    while self.shouldReconnect {
      guard
        await self.sleepUnlessCancelled(
  ...
  private func sendConnect() async throws {
    ...
    let identity =
      includeDeviceIdentity ? DeviceIdentityStore.loadOrCreate(profile: deviceIdentityProfile) : nil
    ...
    do {
      ...
      let shouldRetryWithDeviceToken = self.shouldRetryWithStoredDeviceToken(
        error: error,
        explicitGatewayToken: self.token?.trimmingCharacters(in: .whitespacesAndNewlines)
          .nilIfEmpty,
  ...
  private func selectConnectAuth(
    ...
    let suppressedDeviceTokenRetry =
      includeDeviceIdentity && self.pendingDeviceTokenRetry && requestedScopesExceedStoredToken
      && storedToken != nil && explicitToken != nil
  ...
  private nonisolated static func operatorScopeSatisfied(_ scope: String, granted: Set<String>)
    -> Bool
  {
  ...
  private func persistBootstrapHandoffToken(
    ...
    guard let filteredScopes = self.filteredBootstrapHandoffScopes(role: role, scopes: scopes)
    else {
      return
    }
  ...
  private func handleConnectResponse(
    ...
    if let identity {
      if let deviceToken = auth["deviceToken"]?.value as? String {
        ...
        let scopes =
          (auth["scopes"]?.value as? [ProtoAnyCodable])?
        ...
        let tokenEntries = auth["deviceTokens"]?.value as? [ProtoAnyCodable]
      {
        for entry in tokenEntries {
          ...
          let scopes =
            (rawEntry["scopes"]?.value as? [ProtoAnyCodable])?
            .compactMap { $0.value as? String } ?? []
          self.persistBootstrapHandoffToken(
            deviceId: identity.deviceId,
            role: authRole,
    ...
    return try await AsyncTimeout.withTimeout(
      ...
      operation: { [weak self] in
        ...
        while true {
          ...
          if case .event(let evt) = frame, evt.event == "connect.challenge",
  ...
  private func waitForConnectResponse(reqId: String) async throws -> ResponseFrame {
    ...
    while true {
      ...
      if case .res(let res) = frame, res.id == reqId {
        return res
      }
    }
  }
  ...
  private func watchTicks() async {
    ...
    while self.connected {
      guard await self.sleepUnlessCancelled(nanoseconds: UInt64(tolerance * 1_000_000)) else {
        return
      }
  ...
  private func scheduleReconnect() async {
    ...
    guard await self.sleepUnlessCancelled(nanoseconds: UInt64(delay * 1_000_000_000)) else {
      return
    }
    guard self.shouldReconnect else { return }
    guard !self.reconnectPausedForAuthFailure else { return }
    do {
      ...
      self.logger.error(
        "gateway reconnect failed \(wrapped.localizedDescription, privacy: .public)")
      await self.scheduleReconnect()
    }
  }
  ...
  private func shouldRetryWithStoredDeviceToken(
    ...
    return authError.canRetryWithDeviceToken || authError.detail == .authTokenMismatch
  }
  ...
  public func request(
    ...
    guard case .res(let res) = response else {
      throw NSError(
        domain: "Gateway", code: 2, userInfo: [NSLocalizedDescriptionKey: "unexpected frame"])
    }
    if res.ok == false {
  ...
  private func wrap(_ error: Error, context: String) -> Error {
    if error is GatewayConnectAuthError || error is GatewayResponseError
      || error is GatewayDecodingError || error is GatewayTLSValidationError
    {
      return error
    }
    ...
    return NSError(
      domain: ns.domain, code: ns.code, userInfo: [NSLocalizedDescriptionKey: "\(context): \(desc)"]
    )
  }
