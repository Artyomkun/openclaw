...
func canonicalizeCanvasHostUrl(raw: String?, activeURL: URL?) -> String? {
  ...
  guard let activeURL, let fallbackHost = activeURL.host, !LoopbackHost.isLoopback(fallbackHost)
  else {
    return trimmed
  }
  let isTLS = activeURL.scheme?.lowercased() == "wss"
  ...
  parsed.port =
    ((isTLS && fallbackPort == 443) || (!isTLS && fallbackPort == 80)) ? nil : fallbackPort
  return parsed.string ?? trimmed
}
...
public actor GatewayNodeSession {
  ...
  static func invokeWithTimeout(
    ...
    let response = await withCheckedContinuation {
      (cont: CheckedContinuation<BridgeInvokeResponse, Never>) in
    ...
    timeoutLogger
      .info(
        "node invoke race resolved id=\(request.id, privacy: .public) ok=\(response.ok, privacy: .public)"
      )
    return response
  }
  ...
  private func connectOptionsKey(_ options: GatewayConnectOptions) -> String {
    ...
    let clientDisplayName = (options.clientDisplayName ?? "").trimmingCharacters(
  ...
  public func connect(
    ...
    let shouldReconnect =
      self.activeURL != url || self.activeToken != token
      || self.activeBootstrapToken != bootstrapToken || self.activePassword != password
      || self.activeConnectOptionsKey != nextOptionsKey
      || self.activeSessionIdentity != nextSessionIdentity || self.channel == nil
  ...
  public func request(method: String, paramsJSON: String?, timeoutSeconds: Int = 15) async throws
    -> Data
  {
    guard let channel = self.channel else {
      throw NSError(
  ...
  private func handleEvent(_ evt: EventFrame) async {
    ...
    do {
      ...
      self.logger.error(
        "node invoke decode failed: \(error.localizedDescription, privacy: .public)")
    }
  }

  private func decodeInvokeRequest(from payload: OpenClawProtocol.AnyCodable) throws
  ...
  private func sendInvokeResult(request: NodeInvokeRequestPayload, response: BridgeInvokeResponse)
    async
  {
    guard let channel = self.channel else { return }
    self.logger.info(
