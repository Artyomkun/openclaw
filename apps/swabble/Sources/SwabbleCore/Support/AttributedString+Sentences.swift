...
extension AttributedString {
  public func sentences(maxLength: Int? = nil) -> [AttributedString] {
    ...
    let sentenceRanges = tokenizer.tokens(for: string.startIndex..<string.endIndex).map {
      (
        $0,
        AttributedString.Index($0.lowerBound, within: self)!..<AttributedString.Index(
          $0.upperBound, within: self)!
      )
    }
    let ranges = sentenceRanges.flatMap { sentenceStringRange, sentenceRange in
      ...
      wordRanges[wordRanges.count - 1] =
        wordRanges[wordRanges.count - 1].lowerBound..<sentenceRange.upperBound
  ...
  private func sentenceWithAudioTimeRange(_ range: Range<AttributedString.Index>)
    -> AttributedString?
  {
