...
public struct WakeWordSegment: Sendable, Equatable {
  ...
  public init(
    text: String, start: TimeInterval, duration: TimeInterval, range: Range<String.Index>? = nil
  ) {
    self.text = text
    self.start = start
...
public enum WakeWordGate {
    ...
    -> WakeWordGateMatch?
  {
    ...
    let command = self.commandText(
      transcript: transcript, segments: segments, triggerEndTime: best.triggerEnd
    )
