...
If a named account created an unscoped dynamic agent on an older release, that older agent still counts toward `maxAgents`. Confirm that it is not used by the default account before removing it, or temporarily increase `maxAgents`; OpenClaw cannot safely infer which account owns ambiguous older state.
</Note>
