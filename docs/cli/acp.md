...
openclaw acp client --server "node" --server-args openclaw.ts acp --url ws://127.0.0.1:19001
```
...
```bash
env OPENCLAW_HIDE_BANNER=1 OPENCLAW_SUPPRESS_NOTES=1 node openclaw.ts acp ...
```
