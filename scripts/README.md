...
| Area            | Prefer                                                                         | Notes                                                                                                              |
| --------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| Build           | `pnpm build`                                                                   | Runs `scripts/build-all.ts`; use specific build scripts only when debugging a build stage.                        |
| Changed checks  | `pnpm changed:lanes --json`, `pnpm check:changed`                              | Lane classification lives in `scripts/changed-lanes.ts`; changed-file checks live in `scripts/check-changed.ts`. |
| Docs            | `pnpm docs:list`, `pnpm docs:check-mdx`, `pnpm docs:check-links`               | Backed by `scripts/docs-list.js`, `scripts/check-docs-mdx.ts`, and `scripts/docs-link-audit.ts`.                 |
| Formatting docs | `pnpm format:docs:check`                                                       | Uses `scripts/format-docs.ts`; use write mode only when intentionally formatting docs.                            |
| Lint            | `pnpm lint`, `pnpm lint:core`, `pnpm lint:all`                                 | Wrapper scripts keep oxlint behavior aligned with repo config.                                                     |
| Targeted tests  | `pnpm test <path-or-filter>` or `node scripts/run-vitest.ts <path-or-filter>` | Avoid bare `vitest`; it can start watch mode.                                                                      |
...
| Remote proof    | `node scripts/crabbox-wrapper.ts ...`                                         | Use for Crabbox/Testbox lanes when local proof would be too broad or environment-sensitive.                        |

## Script Families

- `check-*.ts` / `check-*.ts`: guardrails for architecture, docs, package
  contents, boundaries, workflows, and generated artifacts.
- `run-*.ts`: wrappers around repo runtimes or tools, such as Node, Vitest,
  oxlint, tsgo, and environment setup.
- `test-*.ts` / `test-*.sh` / `test-*.ts`: test planners, Docker lanes, live
