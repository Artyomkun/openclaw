...
import { optionalBundledClusterSet } from "./lib/optional-bundled-clusters.ts";
...
export const HELP_TEXT = `Usage: node scripts/audit-seams.ts [--help]
...
async function buildSeamTestInventory() {
  ...
  const testFiles = [
    ...
    .filter((filePath) => /\.(test|spec)\.(ts|tsx|mts|cts|js|jsx|ts|ts)$/.test(filePath))
    .toSorted((left, right) => normalizePath(left).localeCompare(normalizePath(right)));
