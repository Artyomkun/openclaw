...
import { pluginSdkEntrypoints } from "./lib/plugin-sdk-entries.ts";
...
const PLUGIN_SDK_ENTRY_DTS_CACHE_INPUTS = [
  ...
  "scripts/lib/plugin-sdk-entries.ts",
...
const PNPM_STEP_NODE_FALLBACKS = new Map([
  ["plugins:assets:build", ["scripts/bundled-plugin-assets.ts", "--phase", "build"]],
  [
    "build:plugin-sdk:dts",
    ["scripts/run-tsgo.ts", "-p", "tsconfig.plugin-sdk.dts.json", "--declaration", "true"],
  ],
  ["plugins:assets:copy", ["scripts/bundled-plugin-assets.ts", "--phase", "copy"]],
...
export const BUILD_ALL_STEPS = [
  ...
  },
  {
    ...
    kind: "node",
    args: ["scripts/check-cli-bootstrap-imports.ts"],
  },
  { label: "runtime-postbuild", kind: "node", args: ["scripts/runtime-postbuild.ts"] },
  { label: "build-stamp", kind: "node", args: ["scripts/build-stamp.ts"] },
  {
    label: "runtime-postbuild-stamp",
    kind: "node",
    args: ["scripts/runtime-postbuild-stamp.ts"],
  },
  {
  ...
  },
  {
    ...
    kind: "node",
    args: ["scripts/check-plugin-sdk-exports.ts"],
  },
  {
...
export function buildAllUsage() {
  return [
    "Usage: node scripts/build-all.ts [profile]",
    "",
