...
const ROOT_GLOBAL_PATH_RE =
  /^(?:package\.json$|pnpm-lock\.yaml$|pnpm-workspace\.yaml$|vitest\.config\.ts$)/u;
const LIVE_DOCKER_TOOLING_PATH_RE =
  /^(?:scripts\/test-docker-all\.ts|scripts\/test-docker-all\.sh|scripts\/lib\/live-docker-auth\.sh|scripts\/test-live-(?:acp-bind|cli-backend|codex-harness|gateway-models|models)-docker\.sh|src\/gateway\/gateway-acp-bind\.live\.test\.ts|src\/gateway\/live-agent-probes\.test\.ts)$/u;
...
const PUBLIC_EXTENSION_CONTRACT_RE =
  /^(?:src\/plugin-sdk\/|src\/plugins\/contracts\/|src\/channels\/plugins\/|scripts\/lib\/plugin-sdk-entrypoints\.json$|scripts\/sync-plugin-sdk-exports\.ts$|scripts\/generate-plugin-sdk-api-baseline\.ts$)/u;
/**
...
function printUsage() {
  console.log(
    [
      "Usage: node scripts/changed-lanes.ts [options] [-- <paths...>]",
      "",
      "Options:",
