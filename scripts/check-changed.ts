// Runs the changed-file check lanes selected by `scripts/changed-lanes.ts`.
import {
  accessSync,
  chmodSync,
  constants,
...
} from "./changed-lanes.ts";
import { shrinkwrapPackageDirsForChangedPaths } from "./generate-npm-shrinkwrap.ts";
import { booleanFlag, parseFlagArgs, stringFlag } from "./lib/arg-utils.ts";
import { printTimingSummary } from "./lib/check-timing-summary.ts";
import { isDirectRunUrl } from "./lib/direct-run.ts";
import {
  acquireLocalHeavyCheckLockSync,
  resolveLocalHeavyCheckEnv,
} from "./lib/local-heavy-check-runtime.ts";
import { runManagedCommand } from "./lib/managed-child-process.ts";
import { createSparseTsgoSkipEnv } from "./lib/tsgo-sparse-guard.ts";
...
const SHRINKWRAP_POLICY_PATH_RE =
  /^(?:npm-shrinkwrap\.json|package\.json|pnpm-lock\.yaml|pnpm-workspace\.yaml|scripts\/generate-npm-shrinkwrap\.ts|extensions\/[^/]+\/(?:package\.json|npm-shrinkwrap\.json))$/u;
...
const CANVAS_A2UI_NATIVE_RESOURCE_PATH_RE =
  /^(?:pnpm-lock\.yaml$|apps\/shared\/OpenClawKit\/Sources\/OpenClawKit\/Resources\/CanvasA2UI\/|extensions\/canvas\/(?:package\.json$|scripts\/bundle-a2ui\.ts$|src\/host\/a2ui(?:\/(?:index\.html|a2ui\.bundle\.js|\.bundle\.hash)$|-app\/))|scripts\/(?:bundle-a2ui|sync-native-a2ui)\.ts$)/u;
...
export function createShrinkwrapGuardCommand(paths) {
  ...
  return {
    ...
    bin: "node",
    args: [
      "scripts/generate-npm-shrinkwrap.ts",
      "--check",
...
export function createChangedCheckPlan(result, options = {}) {
  ...
  const addTestTempCreationReport = () => {
    ...
    addCommand(
      ...
      "node",
      [
        "scripts/report-test-temp-creations.ts",
        ...(options.staged
          ? ["--staged"]
  ...
  if (shouldRunCanvasA2uiNativeResourceCheck(result.paths)) {
    addCommand(
      ...
      "node",
      ["scripts/sync-native-a2ui.ts", "--check"],
      baseEnv,
    );
  }
  ...
  if (lanes.liveDockerTooling) {
    ...
    addCommand("live Docker scheduler dry run", "node", ["scripts/test-docker-all.ts"], {
      ...baseEnv,
...
function createTargetedOxlintCommand({
  ...
  return {
    ...
    bin: "node",
    args: ["scripts/run-oxlint.ts", "--tsconfig", tsconfig, ...targets],
    env,
  };
}
...
function printUsage() {
  process.stdout.write(
    [
      "Usage: node scripts/check-changed.ts [options] [-- <paths...>]",
      "",
      "Options:",
