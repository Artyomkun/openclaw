...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--help") {
      console.log(
        "Usage: node scripts/check-cli-startup-memory.ts [--json <path>] [--summary <path>]",
      );
      process.exit(0);
    }
...
const cases = [
  ...
  },
  {
    ...
    args: ["openclaw.ts", "gateway", "status"],
    limitMb: readPositiveNumberEnv(
...
function runStartupMemoryCheck(argv = process.argv.slice(2), params = {}) {
  ...
  rssHookPath = path.join(tmpHome, "measure-rss.ts");
  writeFileSync(
    rssHookPath,
    [
      "process.on('exit', () => {",
