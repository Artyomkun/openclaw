...
import { laneResources, laneWeight } from "./lib/docker-e2e-plan.ts";
import { allReleasePathLanes, mainLanes, tailLanes } from "./lib/docker-e2e-scenarios.ts";
...
for (const relativePath of walk("scripts/e2e")) {
  if (!/\.(?:sh|ts|ts|js)$/u.test(relativePath)) {
    continue;
  }
