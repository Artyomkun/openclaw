...
import { stripLeadingPackageManagerSeparator } from "./lib/arg-utils.ts";
import {
  BUILD_STAMP_FILE,
  writeBuildStamp,
  writeRuntimePostBuildStamp,
} from "./lib/local-build-metadata.ts";
import { resolveBuildRequirement } from "./run-node.ts";
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  ...
  const shellSource = [
    ...
    `exec node scripts/watch-node.ts gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
...
function buildRunNodeDeps(env) {
  ...
  return {
    ...
    configFiles: ["tsconfig.json", "package.json", "scripts/build-all.ts"].map((filePath) =>
...
async function main() {
  ...
  if (!options.skipBuild) {
    runCheckedCommand("node", ["scripts/build-all.ts", "gatewayWatch"]);
