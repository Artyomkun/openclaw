...
import {
  ...
  readQaSuiteSummary,
  selectPluginEntries,
} from "./lib/plugin-gateway-gauntlet.ts";
...
export function createGauntletPrebuildCommand(repoRoot) {
  return {
    ...
    args: [path.join(repoRoot, "scripts", "build-all.ts"), "qaRuntime"],
  };
}
...
function sourceOpenclawCommand(repoRoot, args) {
  return {
    command: process.execPath,
    args: [path.join(repoRoot, "scripts", "run-node.ts"), ...args],
  };
}
