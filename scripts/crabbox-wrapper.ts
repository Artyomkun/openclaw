...
import { resolvePathEnvKey, resolveWindowsCmdExePath } from "./windows-cmd-helpers.ts";
...
function enforceCrabboxOwnedBlacksmithLease(commandArgs) {
  ...
  console.error(
    [
      ...
      "[crabbox] create reusable Testboxes through Crabbox before reusing them: node scripts/crabbox-wrapper.ts warmup --provider blacksmith-testbox --idle-timeout 90m",
...
function isChangedGateWords(wordsInput) {
  ...
  return (
    ...
    nodeScriptWord(words)?.endsWith("scripts/check-changed.ts")
  );
}
