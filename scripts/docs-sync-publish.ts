...
import { repairMintlifyAccordionIndentation } from "./lib/mintlify-accordion.ts";
...
const SYNC_SUPPORT_FILES = [
  {
    source: path.join(ROOT, "scripts", "check-docs-mdx.ts"),
    target: path.join(".openclaw-sync", "check-docs-mdx.ts"),
  },
  {
    source: path.join(ROOT, "scripts", "lib", "mintlify-accordion.ts"),
    target: path.join(".openclaw-sync", "lib", "mintlify-accordion.ts"),
  },
  {
