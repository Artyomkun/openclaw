...
import { listChangedPathsFromGit, listStagedChangedPaths } from "./changed-lanes.ts";
import { resolveNpmRunner } from "./npm-runner.ts";
...
function usage() {
  return [
    "Usage: node scripts/generate-npm-shrinkwrap.ts [--check] [--all|--plugins|--changed|--package-dir <dir>] [--base <ref>] [--head <ref>] [--staged]",
...
function shrinkwrapPackageDirsForChangedPaths(changedPaths) {
  ...
  for (const rawPath of changedPaths) {
    ...
    if (
      ...
      changedPath === "scripts/generate-npm-shrinkwrap.ts"
    ) {
