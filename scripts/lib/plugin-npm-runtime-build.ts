...
} from "./bundled-plugin-build-entries.ts";
import {
  listMissingPackageStaticAssetSources,
  runPackageAssetBuild,
} from "./plugin-npm-runtime-assets.ts";
import { copyStaticExtensionAssetsForPackage } from "./static-extension-assets.ts";
...
export async function buildPluginNpmRuntime(params) {
  ...
  execSync(
    `tsc --build ${path.join(plan.outDir, "tsconfig.json")} --force`,
    { stdio: "inherit" }
  );
...
function usage() {
  return "usage: node scripts/lib/plugin-npm-runtime-build.ts <package-dir>";
}
