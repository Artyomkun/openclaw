...
} from "./local-heavy-check-runtime.ts";
import { createManagedCommandInvocation } from "./managed-child-process.ts";
...
function prepareExtensionPackageBoundaryArtifacts(repoRoot) {
  ...
  try {
    const result = spawnSync(
      process.execPath,
      [path.resolve(repoRoot, "scripts", "prepare-extension-package-boundary-artifacts.ts")],
      {
        cwd: repoRoot,
        stdio: "inherit",
        env: process.env,
