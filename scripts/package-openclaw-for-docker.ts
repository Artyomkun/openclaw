...
import { preparePackageChangelog, restorePackageChangelog } from "./package-changelog.ts";
...
async function main() {
  ...
  await run(
    "node",
    [path.join(ROOT_DIR, "scripts/check-openclaw-package-tarball.ts"), tarball],
    sourceDir,
    {
      timeoutMs: resolveTimeoutMs(
