 ...
 *
 * Usage:
 *   node scripts/pre-commit/filter-staged-files.ts lint -- <files...>
 *   node scripts/pre-commit/filter-staged-files.ts format -- <files...>
 *
...
if (mode !== "lint" && mode !== "format") {
  process.stderr.write("usage: filter-staged-files.ts <lint|format> -- <files...>\n");
  process.exit(2);
}

const lintExts = new Set([".ts", ".tsx", ".js", ".jsx", ".ts", ".ts"]);
const formatExts = new Set([".ts", ".tsx", ".js", ".jsx", ".ts", ".ts", ".md", ".mdx"]);
