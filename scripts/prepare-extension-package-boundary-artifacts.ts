...
import { isLocalCheckEnabled } from "./lib/local-heavy-check-runtime.ts";
import { parsePositiveInt } from "./lib/numeric-options.ts";
import { pluginSdkEntrypoints, publicPluginSdkEntrypoints } from "./lib/plugin-sdk-entries.ts";
import { resolveWindowsTaskkillPath } from "./lib/windows-taskkill.ts";

const repoRoot = resolve(import.meta.dirname, "..");
const runTsgoScript = path.join(repoRoot, "scripts/run-tsgo.ts");
const TYPE_INPUT_EXTENSIONS = new Set([".ts", ".tsx", ".d.ts", ".js", ".ts", ".json"]);
