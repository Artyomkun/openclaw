...
import { ensureExtensionMemoryBuild } from "./ensure-extension-memory-build.ts";
import { stripLeadingPackageManagerSeparator } from "./lib/arg-utils.ts";
import { formatErrorMessage } from "./lib/error-format.ts";
...
function printHelp() {
  console.log(`Usage: node scripts/profile-extension-memory.ts [options]
...
async function main() {
  ...
  const hookPath = path.join(tmpHome, "measure-rss.ts");
