...
function usage() {
  return `Usage: node scripts/resolve-openclaw-package-candidate.ts --source <ref|npm|url|trusted-url|artifact> --output-dir <dir> [options]
...
async function resolveCandidate(options) {
  ...
  try {
    if (options.source === "ref") {
      ...
      await run("node", [
        "scripts/package-openclaw-for-docker.ts",
        "--source-dir",
  ...
  await run("node", ["scripts/check-openclaw-package-tarball.ts", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
