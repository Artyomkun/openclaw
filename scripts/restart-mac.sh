...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log "  node openclaw.ts daemon install --force --runtime node"
      log "  node openclaw.ts daemon restart"
      log ""
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node openclaw.ts daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node openclaw.ts daemon restart"
