...
} from "./lib/bundled-plugin-build-entries.ts";
import {
  BUNDLED_PLUGIN_PATH_PREFIX,
  BUNDLED_PLUGIN_ROOT_DIR,
} from "./lib/bundled-plugin-paths.ts";
import {
  BUILD_STAMP_FILE,
...
import {
  ...
  runNodeSourceRoots,
  runNodeWatchedPaths,
} from "./run-node-watch-paths.ts";
import { listCoreRuntimePostBuildOutputs, runRuntimePostBuild } from "./runtime-postbuild.ts";

export { isBuildRelevantRunNodePath, isRestartRelevantRunNodePath, runNodeWatchedPaths };

const buildScript = "scripts/build-all.ts";
const bundledPluginAssetsScript = "scripts/bundled-plugin-assets.ts";
...
const runtimePostBuildWatchedPaths = [
  "scripts/copy-bundled-plugin-metadata.ts",
  "scripts/copy-plugin-sdk-root-alias.ts",
  "scripts/lib",
  "scripts/lib/local-build-metadata.ts",
  "scripts/lib/local-build-metadata-paths.ts",
  "scripts/npm-runner.ts",
  "scripts/runtime-postbuild-stamp.ts",
  "scripts/runtime-postbuild-shared.ts",
  "scripts/runtime-postbuild.ts",
  "scripts/stage-bundled-plugin-runtime.ts",
  "scripts/windows-cmd-helpers.ts",
  "scripts/write-official-channel-catalog.ts",
  "src/plugin-sdk/root-alias.ts",
  BUNDLED_PLUGIN_ROOT_DIR,
];
...
const isRuntimePostBuildRelevantPath = (repoPath) => {
  const normalizedPath = normalizePath(repoPath).replace(/^\.\/+/, "");
  if (normalizedPath === "src/plugin-sdk/root-alias.ts") {
    return true;
  }
...
const runOpenClaw = async (deps) => {
  ...
  const useProcessGroup = shouldUseRunNodeChildProcessGroup(deps);
  const nodeProcess = deps.spawn(deps.execPath, [...diagnosticArgs, "openclaw.ts", ...deps.args], {
    cwd: deps.cwd,
