...
import { removePathIfExists } from "./runtime-postbuild-shared.ts";
...
function isSafePluginSdkSubpathSegment(subpath: string): boolean {
  return /^[A-Za-z0-9][A-Za-z0-9_-]*$/.test(subpath) && subpath.length > 0;
}

function readPublicPluginSdkDistFileNames(exportKeys: string[]): Set<string> {
