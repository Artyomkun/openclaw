#!/usr/bin/env node
// Watches dev source paths and restarts scripts/run-node.ts when relevant
// files change.
...
import { isRestartRelevantRunNodePath, runNodeWatchedPaths } from "./run-node-watch-paths.ts";

const WATCH_NODE_RUNNER = "scripts/run-node.ts";
