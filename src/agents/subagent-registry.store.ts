/**
 * Oracle-backed subagent registry store.
 */
...
export async function loadSubagentRegistry(): Promise<Map<string, SubagentRunRecord>> {
  await ensureSchema();
  return await loadSubagentRuns();
}

export async function saveSubagentRegistry(runs: Map<string, SubagentRunRecord>): Promise<void> {
  await ensureSchema();
  const conn = await getConnection();
  try {
    const runIds: string[] = [];
