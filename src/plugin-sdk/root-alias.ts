...
const pluginSdkSourceExtensions = [".ts", ".ts", ".js", ".ts"];
...
const workspacePackageAliases = [
  {
    ...
    distFile: "dist/index.ts",
  },
  {
    name: "@openclaw/llm-core",
    subpath: "diagnostics",
    srcFile: "src/utils/diagnostics.ts",
    distFile: "dist/utils/diagnostics.ts",
  },
  {
    name: "@openclaw/llm-core",
    subpath: "event-stream",
    srcFile: "src/utils/event-stream.ts",
    distFile: "dist/utils/event-stream.ts",
  },
  {
    name: "@openclaw/llm-core",
    subpath: "types",
    srcFile: "src/types.ts",
    distFile: "dist/types.ts",
  },
  {
    name: "@openclaw/llm-core",
    subpath: "validation",
    srcFile: "src/validation.ts",
    distFile: "dist/validation.ts",
  },
];
const DIAGNOSTIC_EVENTS_STATE_KEY = Symbol.for("openclaw.diagnosticEvents.state.v1");
const isDistRootAlias = __filename.includes(
  `${path.sep}dist${path.sep}plugin-sdk${path.sep}root-alias.ts`,
);
...
function onDiagnosticEvent(listener) {
  ...
  // Keep olders root listeners connected when a built alias resolves the lazy
...
function getModuleLoader(tryNative) {
  ...
  const moduleLoader = createJiti(__filename, {
    ...
    tryNative,
    extensions: [".ts", ".tsx", ".ts", ".tsx", ".js", ".ts", ".json"],
  });
